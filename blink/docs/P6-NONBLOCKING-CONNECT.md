...
- [ ] **N1 — Managed socket contract.** Implement connection state, pending-task
...
- [ ] **N2 — Guest integration.** Carry the contract through authored bridges
  and the translated core. Deliver an actual NativeAOT guest using asynchronous
...
- [ ] **N3 — Qualification and documentation.** Run relevant existing socket,
...
The coordinator assigns non-overlapping ownership for host implementation and
fixtures, integrates bridge changes, serializes shared builds/generation, and
reviews results before declaring completion. Other sessions' changes, especially
...
duration. Existing pinned upstream tests may supply error cases with provenance.
