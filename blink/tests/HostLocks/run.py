#!/usr/bin/env python3
"""Qualify actual private inode/open-description advisory file locks."""
...
base=ROOT/'generated/host-locks';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-locks'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'private shared/exclusive locks, nonblocking contention, duplicate lifetime','passed':False,'results':{}}
...
try:
    for name in ['probe.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostLocks'/name,a/name)
    shutil.copyfile(ROOT/'src/HostIo/HostIoBridge.cs',a/'HostIoBridge.cs')
    shutil.copyfile(ROOT/'src/HostLocks/HostLocksBridge.cs',a/'HostLocksBridge.cs')
    shutil.copyfile(ROOT/'src/HostIo/HostFileControl.c',a/'HostFileControl.c')
    shutil.copytree(ROOT/'config/managed-host',a/'profile')
    shutil.copyfile(ROOT/'src/HostLocks/host-locks.h',a/'profile/host-locks.h')
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_PRIVATE_LOCKS','-I',a/'profile',a/'probe.c',a/'HostFileControl.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostIoBridge.cs','HostLocksBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostLocksProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostLocksProbe')
        project=consumer/'HostLocksProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostLocksProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostLocksProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostLocksProbe')
    ...
    r['passed']=True;save();print('native common and raw/optimized JIT/AOT private file locking checks pass; receipt '+str(out/'receipt.json'))
finally:save()
