#!/usr/bin/env python3
"""Run real file/stream C behavior through native and authored BCL callbacks."""
...
base=ROOT/'generated/host-io';base.mkdir(parents=True,exist_ok=True)
...
try:
    for name in ['probe.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostIo'/name,a/name)
    ...
    (a/'seed').write_bytes(b'abcdef')
    expected=run([a/'native'],'native',30)
    raw=a/'raw';opt=a/'optimized'
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_IO','-I',a/'profile',a/'probe.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostIoBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostIoProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostIoProbe')
        project=consumer/'HostIoProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostIoProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostIoProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostIoProbe')
    ...
    r['passed']=True;save();print('native and raw/optimized JIT/AOT file/stream operations agree; private namespace and captured streams pass')
finally:save()
