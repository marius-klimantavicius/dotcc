#!/usr/bin/env python3
"""Native/common and four managed modes for the actual empty private epoll boundary."""
import hashlib
import json
import os
...
def main():
    base = ROOT / 'artifacts/host-epoll'
    ...
    temporary.mkdir()
    r = dict(passed=False, scope='empty private epoll boundary; no guest service or registered events',
    ...
    try:
        for name in ('run.py', 'probe.c', 'native-main.c', 'layout.c', 'Program.cs'):
            copy(ROOT / 'tests/HostEpoll' / name, snapshot / name)
        for name in ('HostSignals.c', 'HostSignalOps.c', 'HostIoBridge.cs', 'HostEpollBridge.cs'):
        ...
        for label in ('raw', 'optimized'):
            ...
            authored = ('Program.cs', 'HostIoBridge.cs', 'HostEpollBridge.cs')
            ...
            for key, value in (('TargetFramework', 'net10.0'), ('OutputType', 'Exe'),
                               ...
                               ('AssemblyName', 'HostEpollProbe'), ('WarningsAsErrors', 'CS8500')):
            ...
            ET.SubElement(items, 'TrimmerRootAssembly', Include='HostEpollProbe')
            project = consumer / 'HostEpollProbe.csproj'
            ...
            binary = consumer / 'bin/Release/net10.0/HostEpollProbe.dll'
            ...
            execute([publish / 'HostEpollProbe'], publish, label + '-aot', EXPECTED)
