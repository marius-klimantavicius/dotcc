#!/usr/bin/env python3
"""Finite guest TCP syscalls: native and actual managed core."""
import argparse
import hashlib
import json
import os
...
base = ROOT / 'generated/guest-tcp'
base.mkdir(parents=True, exist_ok=True)
a = Path(tempfile.mkdtemp(prefix='attempt-', dir=base))
out = ROOT / 'artifacts/guest-tcp' / a.name
out.mkdir(parents=True)
(out/'tmp').mkdir()
r = {'kind': 'normal-guest-tcp-syscalls', 'passed': False, 'results': {},
     'runner_sha256': sha(Path(__file__)),
     'limitations': ['One actual SYSCALL TCP exchange with valid mapped addresses and buffers; no guest ELF, HTTP, service or execution worker.',
                    'One accept, 257 request bytes, 263 response bytes, orderly EOF; no cancellation, injected failure or malformed input.',
                    'Native libc peer and independent BCL Socket peer verify exact response and EOF. Guest endpoint numbers are private in managed modes.',
                    'One machine lifecycle; canonical objects and Host remain immutable. Native original archive versus canonical managed adapters.']}
...
try:
    ...
    templates = ['probe.c', 'Program.cs', 'peer-native.c']
    r['authored_templates'] = {name: sha(ROOT/'tests/GuestTcp'/name) for name in templates}
    for name in templates:
        shutil.copyfile(ROOT/'tests/GuestTcp'/name, a/name)
    ...
    run(['cc', '-D_GNU_SOURCE', '-D_DEFAULT_SOURCE', '-DNOLINEAR', '-Werror', '-I', native_copy,
         a/'probe.c', a/'peer-native.c', native_copy/'blink.a', '-lz', '-lrt', '-lm', '-pthread', '-o', a/'native'], 'native-adapter-build')
    native_observed = out/'native-adapter.observations.json'
    expected = execute([a/'native', native_observed], 'native-adapter', a/'native')
    required = (b'tcp setup ipv4_loopback=1 ephemeral=1 reuseaddr=1 optlen=4 listener_poll=0\n'
                b'tcp accept count=1 sockaddr_cross_page=1 peer_loopback=1 local_matches=1 nodelay=1 optlen=4\n'
                b'tcp receive bytes=257 exact=1 cross_page=1 canaries=1 eof=1\n'
                b'tcp send bytes=263 cross_page=1 canaries=1 nosignal=1 shutdown_write=1 guest_fds=0 pages=0 syscall_cleanup=1\n'
                b'tcp peer response_bytes=263 exact=1 eof=1 joined=1\n')
    if expected != required:
        raise RuntimeError('Native deterministic TCP contract transcript differs')
    def observations(label, managed):
        path = out/(label+'.observations.json')
        value = json.loads(path.read_text())
        if value['kind'] != ('managed' if managed else 'native'):
            raise RuntimeError('Unexpected endpoint observation kind')
        for name in ['guest_port', 'guest_peer_port', 'physical_port', 'physical_peer_port']:
            if type(value[name]) != int or not 0 < value[name] <= 65535:
                raise RuntimeError('Endpoint outside normal IPv4 TCP port range')
        if value['peer_response_bytes'] != 263 or value['peer_exact'] is not True or value['peer_eof'] is not True:
            raise RuntimeError('Independent peer did not verify exact response/EOF')
        if managed:
            if value['host_descriptors'] != 3 or value['pending_socket_operations'] != 0:
                raise RuntimeError('Private TCP owner did not drain')
        elif value['guest_port'] != value['physical_port'] or value['guest_peer_port'] != value['physical_peer_port']:
            raise RuntimeError('Native endpoints differ from actual socket observations')
        r.setdefault('endpoint_observations', {})[label] = dict(path=str(path), sha256=sha(path), values=value)
        save()
    observations('native-adapter', False)
    r['expected_state_transcript'] = expected.decode()
    r['syscalls'] = {'socket':41, 'setsockopt':54, 'getsockopt':55, 'bind':49, 'getsockname':51,
                     'listen':50, 'poll':7, 'accept':43, 'recvfrom':45, 'sendto':44, 'shutdown':48, 'close':3}
    ...
    if not args.native_only:
        ...
        objects['authored/GuestTcp.c'] = emit('authored/managed-driver.c', a/'probe.c', 'GuestTcp', ['-DGUEST_TCP_MANAGED'])
        ...
        for mode in ['raw', 'optimized']:
            folder = a/mode
            folder.mkdir()
            lib = folder/'GuestTcpLibrary.csproj'
            project(lib, 'GuestTcpLibrary', 'Library', [a/(mode+'-generated')/'*.cs', a/'bridges/*.cs'], [a/'host/Managed.Emulation.Host.csproj'])
            consumer_dir = folder/'consumer'
            consumer_dir.mkdir()
            consumer = consumer_dir/'GuestTcp.csproj'
            project(consumer, 'GuestTcp', 'Exe', [a/'Program.cs'], [lib], 'GuestTcpLibrary')
            if mode == 'optimized':
            ...
            binary = consumer_dir/'bin/Release/net10.0/GuestTcp.dll'
            execute(['dotnet', binary, out/(mode+'-jit.observations.json')], mode+'-jit', binary, managed=True, expected=expected)
            observations(mode+'-jit', True)
            target = folder/'publish'
            run(['dotnet', 'publish', consumer, '-c', 'Release', '-r', 'linux-x64', '-p:PublishAot=true', '-o', target], mode+'-aot-build', 900)
            execute([target/'GuestTcp', out/(mode+'-aot.observations.json')], mode+'-aot', target/'GuestTcp', expected=expected)
            observations(mode+'-aot', True)
        ...
        if any(r['results'].get(label, {}).get('exit_code') != 0 for label in labels):
            raise RuntimeError('Incomplete four-mode guest-tcp matrix')
    ...
    for name, digest in r['authored_templates'].items():
        if sha(ROOT/'tests/GuestTcp'/name) != digest:
    ...
    for row in r['endpoint_observations'].values():
        if sha(Path(row['path'])) != row['sha256']:
            raise RuntimeError('Endpoint observations changed')
    ...
    if sha(Path(__file__)) != r['runner_sha256']:
        raise RuntimeError('GuestTcp runner changed during qualification')
