#!/usr/bin/env python3
"""Native-first and four managed modes for ordinary private socket timeouts."""
import hashlib
import json
import os
...
COMMON = b'timeval16; five-second options; dup inheritance; accept recv read readv recvmsg expiry; normal transfers; zero reset\n'
EXPECTED = COMMON + b'private caller cancellation and owner drain passed\n'
...
def main():
    ...
    base = ROOT / 'artifacts/host-socket-timeouts'
    ...
    temporary.mkdir()
    r = dict(passed=False, scope='ordinary socket timeouts; no forced send expiry or guest service', native_only=args.native_only,
    ...
    try:
        for name in ('run.py', 'probe.c', 'native-main.c', 'Program.cs'):
            copy(ROOT / 'tests/HostSocketTimeouts' / name, snapshot / name)
        for name in ('HostIoBridge.cs', 'HostNetworkBridge.cs', 'HostSocketQueriesBridge.cs', 'HostMessagesBridge.cs'):
            copy(ROOT / 'src/Host' / name, snapshot / name)
        for name in ('host-io.h', 'host-messages.h'):
        ...
        execute([native / 'common'], native, 'native-common', COMMON)
        r['native_passed'] = True
        if args.native_only:
            check()
            r['scope'] = 'native-only ordinary socket timeout witness; managed matrix not run'
            return
        objects = []
        for name in ('probe',):
            output = attempt / (name + '.obj.cs')
            # IncludeRegistry uses the last matching -I; reviewed profile must win.
            run(['dotnet', CLI, '-std=c17', '-DBLINK_MANAGED_SOCKET_TIMEOUTS', '-I', snapshot / 'generic',
                 '-I', snapshot / 'profile', '-I', snapshot / 'headers',
        ...
        for label in ('raw', 'optimized'):
            ...
            authored = ('Program.cs', 'HostIoBridge.cs', 'HostNetworkBridge.cs',
                        'HostSocketQueriesBridge.cs', 'HostMessagesBridge.cs')
            ...
            for key, value in (('TargetFramework', 'net10.0'), ('OutputType', 'Exe'),
                               ...
                               ('AssemblyName', 'HostSocketTimeoutsProbe'), ('WarningsAsErrors', 'CS8500')):
            ...
            ET.SubElement(items, 'TrimmerRootAssembly', Include='HostSocketTimeoutsProbe')
            project = consumer / 'HostSocketTimeoutsProbe.csproj'
            ...
            binary = consumer / 'bin/Release/net10.0/HostSocketTimeoutsProbe.dll'
            ...
            execute([publish / 'HostSocketTimeoutsProbe'], publish, label + '-aot', EXPECTED)
