#!/usr/bin/env python3
"""Run private directory streams through native invariants and authored C callbacks."""
...
base=ROOT/'generated/host-directories';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-directories'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'private directory streams callbacks, not full core or host filesystem' ,'passed':False,'results':{}}
...
try:
    ...
    run(['cc','-std=c17','-D_GNU_SOURCE','-Wall','-Wextra','-Werror','-DBLINK_PROFILE_LAYOUT','-I',a/'profile',a/'layout.c','-o',a/'profile-layout'],'profile-layout-build')
    layout=run([a/'profile-layout'],'profile-layout') # Deliberately supplied profile ABI, distinct from native dirent.
    run(['cc','-std=c17','-D_GNU_SOURCE','-Wall','-Wextra','-Werror',a/'probe.c','-o',a/'native'],'native-build')
    (a/'data').mkdir()
    (a/'data/image').write_bytes(b'alpha');(a/'data/sub').mkdir()

    expected=layout+run([a/'native'],'native',30)
    raw=a/'raw';opt=a/'optimized'
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_DIRECTORIES','-I',a/'profile',a/'probe.c',a/'layout.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostDirectoriesBridge.cs','HostIoBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostDirectoriesProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostDirectoriesProbe')
        project=consumer/'HostDirectoriesProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostDirectoriesProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostDirectoriesProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostDirectoriesProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-directories/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native directory layout/invariants and raw/optimized JIT/AOT private directory streams pass; HostFiles and InstanceIo regressions pass')
finally:save()
