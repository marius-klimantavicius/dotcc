#!/usr/bin/env python3
"""Qualify the actual BCL scheduler hint used by retained host sched_yield calls."""
...
base=ROOT/'generated/host-yield';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-yield'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'actual worker scheduler hint, not priority or realtime scheduling','passed':False,'results':{}}
...
try:
    for name in ['probe.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostYield'/name,a/name)
    shutil.copyfile(ROOT/'src/HostEnvironment/HostEnvironmentBridge.cs',a/'HostEnvironmentBridge.cs')
    shutil.copyfile(ROOT/'src/HostYield/HostYieldBridge.cs',a/'HostYieldBridge.cs')
    shutil.copytree(ROOT/'config/managed-host',a/'profile')
    shutil.copyfile(ROOT/'src/HostYield/host-yield.h',a/'profile/host-yield.h')
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_YIELD','-I',a/'profile',a/'probe.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostEnvironmentBridge.cs','HostYieldBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostYieldProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostYieldProbe')
        project=consumer/'HostYieldProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostYieldProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostYieldProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostYieldProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-yield/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native and raw/optimized JIT/AOT scheduler yield pass')
finally:save()
