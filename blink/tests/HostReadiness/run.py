#!/usr/bin/env python3
"""Run real poll readiness C behavior through native and authored BCL callbacks."""
...
base=ROOT/'generated/host-readiness';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-readiness'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'real authored poll readiness callbacks, not core execution','passed':False,'results':{}}
...
try:
    for name in ['probe.c','Program.cs','native.py']:shutil.copyfile(ROOT/'tests/HostReadiness'/name,a/name)
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostReadinessBridge.cs','HostIoBridge.cs','HostNetworkBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostReadinessProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostReadinessProbe')
        project=consumer/'HostReadinessProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostReadinessProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostReadinessProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostReadinessProbe')
    ...
    r['passed']=True;save();print('native and raw/optimized JIT/AOT poll readiness operations agree; real fragmented clients and simultaneous private ports pass')
finally:save()
