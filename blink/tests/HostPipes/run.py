#!/usr/bin/env python3
"""Run private bounded anonymous pipes through native invariants and authored C callbacks."""
...
base=ROOT/'generated/host-pipes';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-pipes'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'private bounded anonymous pipes callbacks, not full core or host filesystem' ,'passed':False,'results':{}}
...
try:
    for name in ['probe.c','Program.cs','ModelChecks.cs']:shutil.copyfile(ROOT/'tests/HostPipes'/name,a/name)
    shutil.copyfile(ROOT/'src/HostPipes/HostPipesBridge.cs',a/'HostPipesBridge.cs')
    ...
    shutil.copyfile(ROOT/'src/HostPipes/host-pipes.h',a/'profile/host-pipes.h')
    shutil.copyfile(ROOT/'src/HostReadiness/HostReadinessBridge.cs',a/'HostReadinessBridge.cs')
    ...
    r['compiler']=compiler_identity(cli.parent);r['postprocess']={p.name:sha(p) for p in post.parent.iterdir() if p.suffix in ('.dll','.json')};save()
    run(['cc','-std=c17','-D_GNU_SOURCE','-Wall','-Wextra','-Werror',a/'probe.c','-o',a/'native'],'native-build')
    (a/'data').mkdir()
    (a/'data/image').write_bytes(b'alpha');(a/'data/image').chmod(0o444)
    (a/'data/executable').write_bytes(b'ELF');(a/'data/executable').chmod(0o555)
    expected=run([a/'native'],'native',30)
    raw=a/'raw';opt=a/'optimized'
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_PIPES','-I',a/'profile',a/'probe.c',a/'HostFileControl.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    if r['compiler']!=compiler_identity(cli.parent):raise RuntimeError('compiler changed during emission')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','ModelChecks.cs','HostPipesBridge.cs','HostFileMetadataBridge.cs','HostIoBridge.cs','HostReadinessBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostPipesProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostPipesProbe')
        project=consumer/'HostPipesProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostPipesProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostPipesProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostPipesProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-pipes/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native pipe semantics and raw/optimized JIT/AOT private bounded anonymous pipes pass; HostFiles and InstanceIo regressions pass')
finally:save()
