#!/usr/bin/env python3
"""Qualify atomic private descriptor replacement and shared descriptions."""
...
base=ROOT/'generated/host-descriptors';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-descriptors'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'atomic private descriptor replacement','passed':False,'results':{}}
...
try:
    for name in ['probe.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostDescriptors'/name,a/name)
    ...
    shutil.copyfile(ROOT/'src/HostDescriptors/HostDescriptorsBridge.cs',a/'HostDescriptorsBridge.cs')
    shutil.copyfile(ROOT/'src/HostDescriptors/host-descriptors.h',a/'profile/host-descriptors.h')
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_DESCRIPTORS','-I',a/'profile',a/'probe.c',a/'HostFileControl.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostIoBridge.cs','HostDescriptorsBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostDescriptorsProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostDescriptorsProbe')
        project=consumer/'HostDescriptorsProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostDescriptorsProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostDescriptorsProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostDescriptorsProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-descriptors/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native and raw/optimized JIT/AOT descriptor replacement pass; existing file and I/O regressions pass')
finally:save()
