...
base=ROOT/'generated/host-identity-details';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-identity-details'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'real authored process identity callbacks, not core execution','passed':False,'results':{}}
...
try:
    ...
    shutil.copyfile(ROOT/'src/Host/HostIdentityDetailsBridge.cs',a/'HostIdentityDetailsBridge.cs')
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_IDENTITY','-I',a/'profile',a/'probe.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostIdentityBridge.cs','HostIdentityDetailsBridge.cs']:shutil.copyfile(a/name,consumer/name)
    ...
    r['passed']=True;save();print('native and raw/optimized JIT/AOT identity invariants agree; bound private process/credential values remain isolated')
finally:save()
