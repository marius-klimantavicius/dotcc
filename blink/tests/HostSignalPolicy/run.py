#!/usr/bin/env python3
"""Qualify the private disabled-timer and asynchronous-signal refusal contract."""
...
base=ROOT/'generated/host-signal-policy';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-signal-policy'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'private disarmed timers and explicit asynchronous signal refusal','passed':False,'results':{}}
...
try:
    for name in ['probe.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostSignalPolicy'/name,a/name)
    shutil.copyfile(ROOT/'src/HostIdentity/HostIdentityBridge.cs',a/'HostIdentityBridge.cs')
    shutil.copyfile(ROOT/'src/HostSignalPolicy/HostSignalPolicyBridge.cs',a/'HostSignalPolicyBridge.cs')
    shutil.copytree(ROOT/'config/managed-host',a/'profile')
    shutil.copyfile(ROOT/'src/HostSignalPolicy/host-signal-policy.h',a/'profile/host-signal-policy.h')
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_SIGNAL_POLICY','-I',a/'profile',a/'probe.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostIdentityBridge.cs','HostSignalPolicyBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostSignalPolicyProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostSignalPolicyProbe')
        project=consumer/'HostSignalPolicyProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostSignalPolicyProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostSignalPolicyProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostSignalPolicyProbe')
    ...
    r['passed']=True;save();print('native common and raw/optimized JIT/AOT signal policy checks pass; receipt '+str(out/'receipt.json'))
finally:save()
