#!/usr/bin/env python3
"""Run private file metadata through native invariants and authored C callbacks."""
...
base=ROOT/'generated/host-file-metadata';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-file-metadata'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'private file metadata callbacks, not full core or host filesystem' ,'passed':False,'results':{}}
...
try:
    for name in ['probe.c','layout.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostFileMetadata'/name,a/name)
    ...
    if run([a/'profile-layout'],'profile-layout')!=layout:raise RuntimeError('authored/native stat layouts differ')
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_METADATA','-I',a/'profile',a/'probe.c',a/'layout.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostFileMetadataProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostFileMetadataProbe')
        project=consumer/'HostFileMetadataProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostFileMetadataProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostFileMetadataProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostFileMetadataProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-file-metadata/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native stat layout/invariants and raw/optimized JIT/AOT private file metadata pass; HostFiles and InstanceIo regressions pass')
finally:save()
