#!/usr/bin/env python3
"""Normal guest file syscalls: native and actual managed core."""
import argparse
import hashlib
import json
import os
...
base = ROOT / 'generated/guest-io'
base.mkdir(parents=True, exist_ok=True)
a = Path(tempfile.mkdtemp(prefix='attempt-', dir=base))
out = ROOT / 'artifacts/guest-io' / a.name
out.mkdir(parents=True)
(out/'tmp').mkdir()
r = {'kind': 'normal-guest-file-syscalls', 'passed': False, 'results': {},
     'runner_sha256': sha(Path(__file__)),
     'limitations': ['Actual SYSCALL execution for ordinary files/dup/vector I/O/poll; no invalid buffer or fault injection.',
                    'Valid mapped buffers only; no socket/service loop, worker API, permission fault or forced exhaustion.',
...
try:
    ...
    r['authored_templates'] = {name: sha(ROOT/'tests/GuestIo'/name) for name in templates}
    for name in templates:
        shutil.copyfile(ROOT/'tests/GuestIo'/name, a/name)
    ...
    native_file = native_work/'guest-io.bin'
    if native_file.stat().st_size != 131072:
        raise RuntimeError('Unexpected native file size')
    r['native_file'] = {'path': str(native_file), 'sha256': sha(native_file), 'size': native_file.stat().st_size}
    if not expected.startswith(b'cycle=0 file bytes=131072 ') or len(expected.splitlines()) != 10 or b'cycle=1 reopen exact=1 bytes=131072 guest_fds=0 pages=0 syscall_cleanup=1' not in expected:
    ...
    if not args.native_only:
        ...
        objects['authored/GuestIo.c'] = emit('authored/managed-driver.c', a/'probe.c', 'GuestIo', ['-DGUEST_IO_MANAGED'])
        ...
        for mode in ['raw', 'optimized']:
            folder = a/mode
            folder.mkdir()
            lib = folder/'GuestIoLibrary.csproj'
            project(lib, 'GuestIoLibrary', 'Library', [a/(mode+'-generated')/'*.cs', a/'bridges/*.cs'], [a/'host/Managed.Emulation.Host.csproj'])
            consumer_dir = folder/'consumer'
            consumer_dir.mkdir()
            consumer = consumer_dir/'GuestIo.csproj'
            project(consumer, 'GuestIo', 'Exe', [a/'Program.cs'], [lib], 'GuestIoLibrary')
            if mode == 'optimized':
            ...
            binary = consumer_dir/'bin/Release/net10.0/GuestIo.dll'
            ...
            execute([target/'GuestIo'], mode+'-aot', target/'GuestIo', expected=expected)
        ...
        if any(r['results'].get(label, {}).get('exit_code') != 0 for label in labels):
            raise RuntimeError('Incomplete four-mode guest-io matrix')
    ...
    for name, digest in r['authored_templates'].items():
        if sha(ROOT/'tests/GuestIo'/name) != digest:
    ...
    if sha(native_file) != r['native_file']['sha256']:
        raise RuntimeError('Native reference file changed')
    ...
    if sha(Path(__file__)) != r['runner_sha256']:
        raise RuntimeError('GuestIo runner changed during qualification')
