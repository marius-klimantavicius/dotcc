#!/usr/bin/env python3
"""Actual worker/controller processes over a pinned stable threaded delivery."""
import argparse
import glob
import hashlib
import json
...
EXPECTED = b'actual workers: two simultaneous isolated images; ten native HTTP comparisons; cooperative stop; fresh-process restart; idle deadline\n'
...
def main():
    ...
    parser.add_argument('--native-traffic-receipt', type=Path, required=True)
    args = parser.parse_args()
    base = ROOT / 'artifacts/worker-instances'; base.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        native_path = pin(args.native_gc_receipt); native = json.loads(native_path.read_text())
        if not native.get('passed') or not native.get('final_identities_stable') or native['native_exit_code'] != 0:
            raise RuntimeError('Passing actual native GC-profile oracle required')
        expected_env = {'LANG': 'C', 'DOTNET_GCHeapHardLimit': '1000000', 'DOTNET_GCRegionRange': '2000000', 'DOTNET_GCRegionSize': '100000'}
        if native['environment'] != expected_env: raise RuntimeError('Native guest configuration differs')
        ...
        for name in ('health.request', 'health.response', 'stop.request', 'stop.response'):
            shutil.copyfile(native_path.parent / name, oracle / name); pin(oracle / name, native['artifacts'][name])
        elf = pin(native['binary']['path'], native['binary']['sha256'])
        copied_elf = attempt / 'DotNetService'; shutil.copyfile(elf, copied_elf); pin(copied_elf, sha(elf))
        receipt['native'] = dict(path=str(native_path), sha256=sha(native_path), binary=native['binary'], environment=expected_env)
        traffic_path = pin(args.native_traffic_receipt); traffic = json.loads(traffic_path.read_text())
        if (not traffic.get('passed') or not traffic.get('final_identities_stable') or traffic['environment'] != expected_env
            or traffic['producer']['sha256'] != sha(native_path) or traffic['binary'] != native['binary']
            or traffic['exit_code'] != 0 or traffic['cleanup']['signals'] or not traffic['cleanup']['group_gone']):
            raise RuntimeError('Passing exact native traffic witness required')
        if [row['name'] for row in traffic['cases']] != ['large', 'fragmented', 'missing', 'stop'] or not all(row['passed'] for row in traffic['cases']):
            raise RuntimeError('Native traffic case inventory differs')
        if traffic['cases'][1]['write_end_offsets'] != [1, 17, 59, 571, 1595, 3572, 3573]: raise RuntimeError('Fragment schedule differs')
        for name, digest in traffic['inputs'].items(): pin(name, digest)
        for name, digest in traffic['artifacts'].items(): pin(traffic_path.parent / name, digest)
        for kind in ('large', 'fragmented', 'missing'):
            for suffix in ('request', 'response'):
                name = kind + '.' + suffix
                shutil.copyfile(traffic_path.parent / name, oracle / name); pin(oracle / name, traffic['artifacts'][name])
        receipt['native_traffic'] = dict(path=str(traffic_path), sha256=sha(traffic_path), cases=traffic['cases'])
        delivery_path = pin(args.delivery_receipt); delivery = json.loads(delivery_path.read_text())
        ...
        for mode, library, inventory in (('raw', raw, delivery['raw_files']), ('optimized', final, delivery['final_files'])):
            ...
            test_project = private / 'tests/WorkerInstances/WorkerInstances.csproj'
            ...
            test_binary = test_project.parent / 'bin/Release/net10.0/WorkerInstances.dll'
            ...
            for kind in ('jit', 'aot'):
                ...
                try:
                    stdout, stderr = run([dotnet, test_run / test_binary.name, dotnet, executable, copied_elf, oracle, report_dir], label, 180, binaries)
                finally:
                ...
                if fragment['write_end_offsets'] != [1, 17, 59, 571, 1595, 3572, 3573] or fragment['request_bytes'] != 3573:
