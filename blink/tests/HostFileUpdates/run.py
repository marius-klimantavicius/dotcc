#!/usr/bin/env python3
"""Qualify private positioned writes, resizing and in-memory synchronization."""
...
base=ROOT/'generated/host-file-updates';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-file-updates'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'private bounded file updates, not persistent host storage','passed':False,'results':{}}
...
try:
    for name in ['probe.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostFileUpdates'/name,a/name)
    ...
    shutil.copyfile(ROOT/'src/HostFileUpdates/HostFileUpdatesBridge.cs',a/'HostFileUpdatesBridge.cs')
    shutil.copyfile(ROOT/'src/HostFileUpdates/host-file-updates.h',a/'profile/host-file-updates.h')
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_UPDATES','-I',a/'profile',a/'probe.c',a/'HostFileControl.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostFileMetadataBridge.cs','HostIoBridge.cs','HostFileUpdatesBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostFileUpdatesProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostFileUpdatesProbe')
        project=consumer/'HostFileUpdatesProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostFileUpdatesProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostFileUpdatesProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostFileUpdatesProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-file-updates/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native and raw/optimized JIT/AOT file updates pass; HostFiles and InstanceIo regressions pass')
finally:save()
