#!/usr/bin/env python3
"""Run private LIFO exit callbacks through native and authored BCL callbacks."""
...
base=ROOT/'generated/host-exit-callbacks';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-exit-callbacks'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'real authored private bounded exit callback lifecycle, not core execution','objectLink':args.object_link,'passed':False,'results':{}}
...
try:
    for name in ['probe.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostExitCallbacks'/name,a/name)
    shutil.copytree(ROOT/'config/managed-host',a/'profile')
    for name in ['HostExitCallbacks.c','HostExitCallbacks.h']:shutil.copyfile(ROOT/'src/HostExitCallbacks'/name,a/name)
    shutil.copyfile(ROOT/'config/managed-host/host-errors.h',a/'host-errors.h')
    ...
    run(['cc','-std=c17','-Wall','-Wextra','-Werror','-DBLINK_PRIVATE_EXIT_CALLBACKS',a/'probe.c',a/'HostExitCallbacks.c','-o',a/'native-staged'],'native-staged-build')
    ...
    if args.object_link:
        objects=[]
        for name in ['probe','HostExitCallbacks']:
            target=a/(name+'.obj.cs');objects.append(target)
            run(['dotnet',cli,'-std=c17','-DBLINK_PRIVATE_EXIT_CALLBACKS','-DBLINK_MANAGED_EXIT_CALLBACKS','-I',a/'profile',a/(name+'.c'),'--emit=obj','-o',target],'object-'+name)
        run(['dotnet',cli,*objects,'--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'link')
    else:
        run(['dotnet',cli,'-std=c17','-DBLINK_PRIVATE_EXIT_CALLBACKS','-DBLINK_MANAGED_EXIT_CALLBACKS','-I',a/'profile',a/'probe.c',a/'HostExitCallbacks.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostExitCallbacksProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        items=ET.SubElement(xml,'ItemGroup');ET.SubElement(items,'Compile',Include=str(generated/'*.cs'))
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostExitCallbacksProbe')
        project=consumer/'HostExitCallbacksProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostExitCallbacksProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostExitCallbacksProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostExitCallbacksProbe')
    ...
    r['passed']=True;save()
    (ROOT/('artifacts/host-exit-callbacks/latest-object.json' if args.object_link else 'artifacts/host-exit-callbacks/latest.json')).write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native/staged and raw/optimized JIT/AOT private exit callbacks pass; LIFO, bounds, GC and escaped-callback state verified')
finally:save()
