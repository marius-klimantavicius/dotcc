...
def main():
    ...
    artifacts = BLINK / "artifacts/kestrel-guest-musl"
    artifacts.mkdir(parents=True, exist_ok=True)
    attempt = Path(tempfile.mkdtemp(prefix="attempt-", dir=artifacts))
    receipt = {"passed": False, "attempt": str(attempt), "commands": [],
               "scope": "one standard static-musl Kestrel publish and native Linux HTTP; no Blink execution",
    ...
    container = "blink-kestrel-" + attempt.name.removeprefix("attempt-").replace("_", "-")
    ...
    try:
        ...
        for relative, target in (
                ("blink/tests/KestrelService/Program.cs", "source/Program.cs"),
                ("blink/tests/KestrelService/KestrelService.csproj", "source/KestrelService.csproj"),
                ("blink/tests/KestrelService/global.json", "original-global.json"),
...
dotnet publish KestrelService.csproj -c Release -r linux-musl-x64 --self-contained true \\
...
'''
        ...
        if f"DOTNET_SDK_VERSION={SDK}" not in image_env or f"DOTNET_VERSION={RUNTIME}" not in image_env or f"ASPNET_VERSION={RUNTIME}" not in image_env:
        ...
        binary = attempt / "publish/KestrelService"
        ...
        native_oracle(attempt, binary, receipt, GUEST_ENVIRONMENT)
...
GUEST_ENVIRONMENT = {"LANG": "C", "DOTNET_GCHeapHardLimit": "1000000",
                     "DOTNET_GCRegionRange": "2000000", "DOTNET_GCRegionSize": "100000"}


def response_semantics(response):
    head, separator, body = response.partition(b"\r\n\r\n")
    if not separator:
        raise RuntimeError("incomplete HTTP response")
    lines = head.decode("ascii").split("\r\n")
    match = re.fullmatch(r"HTTP/1\.1 ([0-9]{3}) (.+)", lines[0])
    if not match:
    ...
    for line in lines[1:]:
        ...
        if not colon or key in headers:
            raise RuntimeError("invalid or duplicate response header")
        headers[key] = value.strip()
    date = headers.pop("date", None)
    if date is None:
        raise RuntimeError("Kestrel Date header missing")
    parsed = parsedate_to_datetime(date)
    if parsed.tzinfo is None or parsed.strftime("%a, %d %b %Y %H:%M:%S GMT") != date:
        raise RuntimeError("invalid RFC1123 Date header")
    if headers.get("content-length") != str(len(body)):
        raise RuntimeError("response Content-Length does not match body")
    return {"version": "HTTP/1.1", "status": int(match[1]), "reason": match[2],
            "headers": dict(sorted(headers.items())), "body_hex": body.hex(),
            "date": date, "date_valid_rfc1123": True,
            "comparison": "Compare every semantic field except the validated actual Date value; retain complete raw response."}


def cleanup_native_group(process, receipt):
    def exists():
        try:
    ...
    row["group_gone"] = not exists()
    row["normal"] = row["group_gone"] and row["leader_exit_code"] == 0 and not row["signals"]
...
def native_oracle(attempt, binary, receipt, env):
    ...
    receipt["native_environment"] = dict(env)
    receipt["native_limits"] = {"overall_seconds": 90, "ready_seconds": 20, "response_bytes": 8192}
    signal.alarm(90)
    captured = bytearray()
    communicated = False
    with (attempt / "native.stderr").open("wb") as errors:
        ...
        try:
            ...
            for name, method, path, status, body, padding, fragmented in (
                    ("health", "GET", "/health", 200, b"ok\n", 0, False),
                    ("large", "GET", "/health", 200, b"ok\n", 3500, False),
                    ("fragmented", "GET", "/health", 200, b"ok\n", 3500, True),
                    ("missing", "GET", "/missing", 404, b"not found\n", 0, False),
                    ("stop", "POST", "/stop", 200, b"stopped\n", 0, False)):
                request = f"{method} {path} HTTP/1.1\r\nHost: localhost\r\nContent-Length: 0\r\nConnection: close\r\n".encode("ascii")
                if padding:
                    request += b"X-Padding: " + b"a" * padding + b"\r\n"
                request += b"\r\n"
                response = bytearray()
                (attempt / (name + ".request")).write_bytes(request)
                writes = [1, 17, 59, 571, 1595, len(request)-1, len(request)] if fragmented else [len(request)]
                try:
                    with socket.create_connection(("127.0.0.1", int(match[1])), timeout=5) as client:
                        client.settimeout(5)
                        offset = 0
                        for end in writes:
                            client.sendall(request[offset:end])
                            offset = end
                ...
                semantic = response_semantics(bytes(response))
                if (semantic["status"] != status or bytes.fromhex(semantic["body_hex"]) != body
                        or semantic["headers"].get("content-type") != "text/plain"
                        or semantic["headers"].get("content-length") != str(len(body))
                        or semantic["headers"].get("connection") != "close"
                        or semantic["headers"].get("server") != "Kestrel"):
                    raise RuntimeError(f"native {name} response mismatch: {semantic}")
                receipt["native_cases"].append({"name": name, "path": path, "passed": True,
                    "request_bytes": len(request), "write_end_offsets": writes,
                    "semantic": semantic,
                    "request_sha256": sha(attempt / (name + ".request")),
                    "response_sha256": sha(attempt / (name + ".response"))})
            ...
            signal.alarm(0)
            cleanup_native_group(process, receipt)
            if not communicated:
                try:
            ...
            if trace.exists():
                ...
                receipt["native_observed_tids"] = sorted(set(map(int, re.findall(r"^(\d+)\s", trace.read_text(), re.M))))
                receipt["trace_syscalls"] = sorted(set(re.findall(r"^\d+\s+(\w+)\(", trace.read_text(), re.M)))

            if not receipt["native_cleanup"]["normal"]:
                raise RuntimeError("native process group required forced cleanup or did not drain")
