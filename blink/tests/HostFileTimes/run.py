#!/usr/bin/env python3
"""Run private exact file timestamps through native invariants and authored C callbacks."""
...
base=ROOT/'generated/host-file-times';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-file-times'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'private exact file timestamps callbacks, not full core or host filesystem' ,'passed':False,'results':{}}
...
try:
    for name in ['probe.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostFileTimes'/name,a/name)
    shutil.copyfile(ROOT/'src/HostFileTimes/HostFileTimesBridge.cs',a/'HostFileTimesBridge.cs')
    ...
    shutil.copyfile(ROOT/'src/HostFileTimes/host-file-times.h',a/'profile/host-file-times.h')
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_TIMES','-I',a/'profile',a/'probe.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostFileTimesBridge.cs','HostFileMetadataBridge.cs','HostIoBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostFileTimesProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostFileTimesProbe')
        project=consumer/'HostFileTimesProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostFileTimesProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostFileTimesProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostFileTimesProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-file-times/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native timestamp layout/semantics and raw/optimized JIT/AOT private exact file timestamps pass; HostFiles and InstanceIo regressions pass')
finally:save()
