...
base=ROOT/'generated/host-identity';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-identity'/a.name;out.mkdir(parents=True)
...
try:
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostIdentityBridge.cs']:shutil.copyfile(a/name,consumer/name)
