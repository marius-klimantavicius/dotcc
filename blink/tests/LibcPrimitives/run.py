#!/usr/bin/env python3
"""Native comparison and forced-GC JIT/AOT qualification of generic string/math primitives."""
import hashlib
import json
import os
...
REPO = ROOT.parent
base = ROOT / 'generated/libc-primitives'
base.mkdir(parents=True, exist_ok=True)
attempt = Path(tempfile.mkdtemp(prefix='attempt-', dir=base))
out = ROOT / 'artifacts/libc-primitives' / attempt.name
...
fixture_name = 'libc-copy-round'
fixture = REPO / 'DotCC.FunctionalTests/Fixtures' / fixture_name / 'main.c'
source = attempt / 'main.c'
shutil.copy2(fixture, source)
hooks = attempt / 'GcHooks.cs'
shutil.copy2(ROOT / 'tests/LibcPrimitives/GcHooks.cs', hooks)
sha = lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
receipt = dict(scope='generic-string-copy-and-default-nearest-rounding-no-mutable-fenv',
...
receipt['repositoryHead'] = subprocess.check_output(['git', '-C', str(REPO), 'rev-parse', 'HEAD'], text=True).strip()
receipt['cliSha256'] = sha(cli)
receipt['runnerSha256'] = sha(Path(__file__))
receipt['runtimeSources'] = {str(p.relative_to(REPO)):sha(p) for p in [REPO/'DotCC.Libc/StringCopyLib.cs', REPO/'DotCC.Libc/MathRoundLib.cs', REPO/'DotCC.Lib/include/string.h', REPO/'DotCC.Lib/include/math.h']}
...
def create_consumer(generated, variant):
    ...
    project_path = consumer / 'LibcPrimitivesGc.csproj'
...
try:
    ...
    run(['cc', '-std=c17', '-Wall', '-Wextra', '-Werror', '-fno-builtin', str(source), '-lm', '-o', str(native)], 'native-build')
    ...
    for variant, input_dir in [('raw', generated), ('optimized', optimized)]:
        ...
        actual = run(['dotnet', str(consumer / 'bin/Release/net10.0/LibcPrimitivesGc.dll')], variant + '-jit', 30)
        if actual != expected:
        ...
        actual = run([str(publish / 'LibcPrimitivesGc')], variant + '-aot', 30)
        if actual != expected:
