#!/usr/bin/env python3
"""Qualify bounded ancillary control-record traversal against native libc."""
...
base=ROOT/'generated/host-ancillary';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-ancillary'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'control record traversal only, not ancillary socket transport','passed':False,'results':{}}
...
try:
    ...
    run(['cc','-std=c17','-Wall','-Wextra','-Werror','-DBLINK_PRIVATE_ANCILLARY',a/'probe.c','-o',a/'native-private'],'native-private-build')
    if run([a/'native-private'],'native-private')!=expected:raise RuntimeError('native authored traversal differs from libc')
    raw=a/'raw';opt=a/'optimized'
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_ANCILLARY','-DBLINK_PRIVATE_ANCILLARY','-I',a/'profile',a/'probe.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostAncillaryProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        items=ET.SubElement(xml,'ItemGroup');ET.SubElement(items,'Compile',Include=str(generated/'*.cs'))
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostAncillaryProbe')
        project=consumer/'HostAncillaryProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostAncillaryProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostAncillaryProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostAncillaryProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-ancillary/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native and raw/optimized JIT/AOT control-record traversal pass')
finally:save()
