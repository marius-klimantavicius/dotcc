#!/usr/bin/env python3
"""Normal guest clock, entropy, identity and signal-state syscalls."""
import argparse
import hashlib
import json
import os
...
base = ROOT / 'generated/guest-environment'
base.mkdir(parents=True, exist_ok=True)
a = Path(tempfile.mkdtemp(prefix='attempt-', dir=base))
out = ROOT / 'artifacts/guest-environment' / a.name
out.mkdir(parents=True)
(out/'tmp').mkdir()
r = {'kind': 'normal-guest-environment-syscalls', 'passed': False, 'results': {},
     'runner_sha256': sha(Path(__file__)),
     'limitations': ['Actual SYSCALL clock/random/tid/signal-state execution; no delivered signal or fault injection.',
                    'Raw observations retained; no timestamp/random equality, entropy quality, timer, stop or worker claim.',
...
def normalize_observations(value, label, wall_before_ns, wall_after_ns):
    """Validate every raw row; compare only the specified common contracts."""
    lines = value.decode('ascii').splitlines()
    managed = label != 'native-adapter'
    frequency = None
    if managed:
        match = re.fullmatch(r'provider timestamp_frequency=([1-9][0-9]*)', lines.pop(0) if lines else '')
        if not match:
            raise RuntimeError(label + ' lacks the actual TimeProvider frequency')
        frequency = int(match[1])
    if len(lines) != 20:
        raise RuntimeError(label + ' observation inventory differs')
    # A wall-clock correction exceeding this bracket fails qualification; no
    # realtime monotonicity or equality between independently run processes.
    ...
    def state(expected):
        actual = lines.pop(0)
        if actual != expected:
            raise RuntimeError(label + ' state row differs: ' + actual)
        states.append(actual)

    def observation(pattern):
        actual = lines.pop(0)
        match = re.fullmatch(pattern, actual)
        if not match:
            raise RuntimeError(label + ' observation row differs: ' + actual)
        return match

    for cycle in range(2):
        for clock in range(2):
            match = observation(rf'obs cycle={cycle} clock={clock} first_sec=([0-9]+) first_nsec=([0-9]+) second_sec=([0-9]+) second_nsec=([0-9]+) res_sec=([0-9]+) res_nsec=([0-9]+)')
            first_sec, first_nsec, second_sec, second_nsec, res_sec, res_nsec = map(int, match.groups())
            if any(n >= 1_000_000_000 for n in (first_nsec, second_nsec, res_nsec)):
                raise RuntimeError(label + ' timespec is not normalized')
            first = first_sec * 1_000_000_000 + first_nsec
            second = second_sec * 1_000_000_000 + second_nsec
            resolution = res_sec * 1_000_000_000 + res_nsec
            if resolution <= 0 or clock == 1 and second < first:
                raise RuntimeError(label + ' clock contract differs')
            if clock == 0 and not all(wall_before_ns - tolerance_ns <= stamp <= wall_after_ns + tolerance_ns for stamp in (first, second)):
                raise RuntimeError(label + ' realtime is outside its execution bracket')
            if managed:
                expected_resolution = 100 if clock == 0 else max(100, (1_000_000_000 + frequency - 1) // frequency)
                if resolution != expected_resolution or first_nsec % 100 or second_nsec % 100:
                    raise RuntimeError(label + ' private provider output quantum differs')
            observations.append(dict(cycle=cycle, kind='clock', clock=clock,
                                     first_sec=first_sec, first_nsec=first_nsec,
                                     second_sec=second_sec, second_nsec=second_nsec,
                                     res_sec=res_sec, res_nsec=res_nsec))
        state(f'state cycle={cycle} clocks=2 normalized=1 monotonic_nondecrease=1 resolution_positive=1')
        match = observation(rf'obs cycle={cycle} random=([0-9a-f]{{64}})')
        observations.append(dict(cycle=cycle, kind='random', bytes_hex=match[1]))
        state(f'state cycle={cycle} random_return=32 cross_page=1 canaries=1')
        match = observation(rf'obs cycle={cycle} tid=([1-9][0-9]*) ctid=([0-9a-f]+)')
        tid, ctid = int(match[1]), int(match[2], 16)
        if ctid != 0x600280 or managed and tid != 73:
            raise RuntimeError(label + ' private identity/address contract differs')
        observations.append(dict(cycle=cycle, kind='tid', tid=tid, ctid=ctid))
        state(f'state cycle={cycle} tid_matches=1 ctid_stored=1 word_unchanged=1')
        match = observation(rf'obs cycle={cycle} initial_mask=([0-9a-f]+) host_ignore_flags=([0-9a-f]+) host_default_flags=([0-9a-f]+)')
        initial_mask, ignore_flags, default_flags = (int(item, 16) for item in match.groups())
        allowed_flags = 4 if managed else 4 | 0x04000000
        if initial_mask or any(not(flags & 4) or flags & ~allowed_flags for flags in (ignore_flags, default_flags)):
            raise RuntimeError(label + ' host action flags or initial guest mask differ')
        observations.append(dict(cycle=cycle, kind='signal', initial_mask=initial_mask,
                                 host_ignore_flags=ignore_flags, host_default_flags=default_flags))
        state(f'state cycle={cycle} disposition_restored=1 guest_mask_restored=1 host_registry=1 pending=0 delivered=0')
        state(f'state cycle={cycle} cleanup pages=0 guest_fds=0 syscall_cleanup=1')
    if lines:
        raise RuntimeError(label + ' has additional unreviewed observations')
    r['results'][label]['observations'] = observations
    r['results'][label]['comparison'] = dict(wall_before_ns=wall_before_ns, wall_after_ns=wall_after_ns,
                                            realtime_tolerance_ns=tolerance_ns,
                                            timestamp_frequency=frequency,
                                            normalized_state_lines=states)
    return ('\n'.join(states) + '\n').encode()


def run(command, label, timeout=300, cwd=None):
    command = list(map(str, command))
    start, code = time.monotonic(), None
    stdout_path, stderr_path = out / (label + '.stdout'), out / (label + '.stderr')
    try:
        with stdout_path.open('wb') as stdout, stderr_path.open('wb') as stderr:
            process = subprocess.Popen(command, cwd=cwd or ROOT.parent, stdout=stdout, stderr=stderr,
        ...
        r['results'][label] = {'command': command, 'cwd': str(cwd or ROOT.parent), 'exit_code': code, 'seconds': time.monotonic()-start,
...
def execute(command, label, binary, managed=False, expected=None, cwd=None):
    ...
    value = run(command, label, 30, cwd=cwd)
    ...
    r['results'][label].update(binaries_before=before, binaries_after=after,
                               wall_before_ns=wall_before_ns, wall_after_ns=wall_after_ns)
    save()
    if before != after:
    ...
    value = normalize_observations(value, label, wall_before_ns, wall_after_ns)
...
try:
    ...
    r['authored_templates'] = {name: sha(ROOT/'tests/GuestEnvironment'/name) for name in templates}
    for name in templates:
        shutil.copyfile(ROOT/'tests/GuestEnvironment'/name, a/name)
    ...
    if len(expected.splitlines()) != 10 or b'state cycle=1 cleanup pages=0 guest_fds=0 syscall_cleanup=1' not in expected:
    ...
    if not args.native_only:
        ...
        objects['authored/GuestEnvironment.c'] = emit('authored/managed-driver.c', a/'probe.c', 'GuestEnvironment', ['-DGUEST_ENVIRONMENT_MANAGED'])
        ...
        for mode in ['raw', 'optimized']:
            folder = a/mode
            folder.mkdir()
            lib = folder/'GuestEnvironmentLibrary.csproj'
            project(lib, 'GuestEnvironmentLibrary', 'Library', [a/(mode+'-generated')/'*.cs', a/'bridges/*.cs'], [a/'host/Managed.Emulation.Host.csproj'])
            consumer_dir = folder/'consumer'
            consumer_dir.mkdir()
            consumer = consumer_dir/'GuestEnvironment.csproj'
            project(consumer, 'GuestEnvironment', 'Exe', [a/'Program.cs'], [lib], 'GuestEnvironmentLibrary')
            if mode == 'optimized':
            ...
            binary = consumer_dir/'bin/Release/net10.0/GuestEnvironment.dll'
            ...
            execute([target/'GuestEnvironment'], mode+'-aot', target/'GuestEnvironment', expected=expected)
        ...
        if any(r['results'].get(label, {}).get('exit_code') != 0 for label in labels):
            raise RuntimeError('Incomplete four-mode guest-environment matrix')
    ...
    for name, digest in r['authored_templates'].items():
        if sha(ROOT/'tests/GuestEnvironment'/name) != digest:
    ...
    if sha(Path(__file__)) != r['runner_sha256']:
        raise RuntimeError('GuestEnvironment runner changed during qualification')
