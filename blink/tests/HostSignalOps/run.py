#!/usr/bin/env python3
"""Real POSIX set and virtual-mask operations with unchanged host-thread masks."""
import hashlib
import json
import os
...
REPO = ROOT.parent
base = ROOT / 'generated/host-signal-ops'
base.mkdir(parents=True, exist_ok=True)
attempt = Path(tempfile.mkdtemp(prefix='attempt-', dir=base))
out = ROOT / 'artifacts/host-signal-ops' / attempt.name
...
ops = source_dir / 'HostSignalOps.c'
hooks = ROOT / 'tests/HostSignals/Hooks.cs'
sha = lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
receipt = dict(scope='public-signal-sets-and-virtual-host-mask-not-guest-signal-delivery',
...
def create_consumer(generated, variant):
    ...
    ET.SubElement(items, 'TrimmerRootAssembly', Include='HostSignals')
...
try:
    ...
    run(['cc', '-std=c17', '-Wall', '-Wextra', '-Werror', '-DBLINK_HOST_NATIVE_ORACLE',
         '-iquote', str(source_dir), str(source), str(adapter), str(ops), '-o', str(contract)], 'native-contract-build')
    ...
    run(['dotnet', str(cli), '-std=c17', '-DBLINK_TEST_MANAGED',
         '-I', str(source_dir), '-I', str(profile), str(source), str(adapter), str(ops),
    ...
    save()
    print(f'Signal sets and virtual masks match real POSIX and native contract under raw/optimized JIT/NativeAOT; host mask unchanged and CS8500 forbidden. Receipt: {out / "receipt.json"}')
