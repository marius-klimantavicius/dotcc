#!/usr/bin/env python3
"""Run real process identity C behavior through native and authored BCL callbacks."""
...
base=ROOT/'generated/host-calendar';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-calendar'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'instance time and explicit UTC calendar, not core execution','passed':False,'results':{}}
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def save():(out/'receipt.json').write_text(json.dumps(r,indent=2)+'\n')
def run(cmd,name,timeout=180):
    with(out/(name+'.log')).open('wb')as log: code=subprocess.run(list(map(str,cmd)),stdout=log,stderr=subprocess.STDOUT,env=dict(os.environ,LC_ALL='C',TZ='UTC0'),timeout=timeout).returncode
...
try:
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_CALENDAR','-I',a/'profile',a/'probe.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostEnvironmentBridge.cs','HostCalendarBridge.cs']:shutil.copyfile(a/name,consumer/name)
    ...
    r['passed']=True;save();print('native and raw/optimized JIT/AOT UTC calendar and instance time pass')
finally:save()
