#!/usr/bin/env python3
"""One bounded scalar and SIMD register observation of the immutable Kestrel baseline."""
...
_CAMPAIGN_INPUTS = _campaign_json.loads((_CAMPAIGN_ROOT / "config/script-inputs.json").read_text())['tests/KestrelGuestExecution/float-register-assessment.py']
...
def main():
    ...
    attempt = Path(tempfile.mkdtemp(prefix="attempt-float-registers-", dir=BASE.parent))
    (attempt / "tmp").mkdir()
    receipt = dict(passed=False, diagnostic_completed=False, inputs={}, commands=[],
                   ...
                   scope="One diagnostic-only authored owner observation; unchanged immutable guest/generated/Host")
    ...
    try:
        ...
        pin(__file__)
        shutil.copyfile(__file__, attempt / "float-register-assessment.py")
        pin(attempt / "float-register-assessment.py", sha(__file__))
        pin(sys.executable)
        before_trees = {}
        ...
        marker = '                writer.WriteBoolean("syscall_trace_available", true);'
        if after.count(marker) != 1: raise RuntimeError("Result writer marker differs")
        after = after.replace(marker, marker + '\n                owner.WriteDiagnosticRegisters(writer);')
        program.write_text(after)
        owner = attempt / "private/src/Managed.Emulation.ThreadedExecution/ThreadedGuestExecution.cs"
        owner_before = owner.read_text()
        owner_after = owner_before
        replacements = [
            ('public sealed unsafe class ThreadedGuestExecution : IHostGuestThreads',
             'public sealed unsafe partial class ThreadedGuestExecution : IHostGuestThreads'),
            ('                try { Blink.ExecuteInstruction(machine); returned = true; }',
             '                try { ObserveDiagnosticRegisters(worker); Blink.ExecuteInstruction(machine); returned = true; }'),
            ('        finally { worker.Ip = machine->ip; ClearExecution(worker); }',
             '        finally { worker.Ip = machine->ip; ObserveDiagnosticRegisters(worker, terminal: true); ClearExecution(worker); }')]
        for before_line, after_line in replacements:
            if owner_after.count(before_line) != 1: raise RuntimeError("Owner observation marker differs")
            owner_after = owner_after.replace(before_line, after_line)
        owner.write_text(owner_after)
        observer_source = pin(Path(__file__).with_name("FloatRegisterObservation.cs.txt"))
        observer = owner.with_name("RegisterObservation.cs")
        shutil.copyfile(observer_source, observer)
        receipt["owner_derivation"] = dict(path=str(owner.relative_to(attempt)),
            before_sha256=hashlib.sha256(owner_before.encode()).hexdigest(), after_sha256=sha(owner),
        ...
        expected_private["src/Managed.Emulation.ThreadedExecution/ThreadedGuestExecution.cs"] = sha(owner)
        expected_private["src/Managed.Emulation.ThreadedExecution/RegisterObservation.cs"] = sha(observer)
        ...
        if result_path.exists():
            ...
            receipt["passed"] = bool(receipt["commands"][-1]["exit_code"] == 0 and
                ...
                result.get("register_observations", {}).get("error") is None and
