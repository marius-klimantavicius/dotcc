...
def main():
    ...
    attempt = Path(tempfile.mkdtemp(prefix="attempt-registers-", dir=BASE.parent))
    (attempt / "tmp").mkdir()
    ...
    try:
        ...
        pin(__file__)
        shutil.copyfile(__file__, attempt / "register-assessment.py")
        pin(attempt / "register-assessment.py", sha(__file__))
        pin(sys.executable)
        before_trees = {}
        ...
        observer_source = pin(Path(__file__).with_name("RegisterObservation.cs.txt"))
