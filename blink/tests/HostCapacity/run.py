#!/usr/bin/env python3
"""Run private filesystem capacity through native invariants and authored C callbacks."""
...
base=ROOT/'generated/host-capacity';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-capacity'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'private filesystem capacity callbacks, not full core or host filesystem' ,'passed':False,'results':{}}
...
try:
    for name in ['probe.c','layout.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostCapacity'/name,a/name)
    shutil.copyfile(ROOT/'src/HostCapacity/HostCapacityBridge.cs',a/'HostCapacityBridge.cs')
    shutil.copytree(ROOT/'config/managed-host',a/'profile')
    shutil.copyfile(ROOT/'src/HostIo/host-io.h',a/'profile/host-io.h')
    shutil.copyfile(ROOT/'src/HostCapacity/host-capacity.h',a/'profile/host-capacity.h')
    ...
    run(['cc','-std=c17','-D_GNU_SOURCE','-Wall','-Wextra','-Werror',a/'layout.c','-o',a/'native-layout'],'native-layout-build')
    layout=run([a/'native-layout'],'native-layout')
    run(['cc','-std=c17','-D_GNU_SOURCE','-Wall','-Wextra','-Werror','-I',a/'profile',a/'layout.c','-o',a/'profile-layout'],'profile-layout-build')
    if run([a/'profile-layout'],'profile-layout')!=layout:raise RuntimeError('authored/native statvfs layouts differ')
    ...
    expected=layout+run([a/'native'],'native',30)
    raw=a/'raw';opt=a/'optimized'
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_CAPACITY','-I',a/'profile',a/'probe.c',a/'layout.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostCapacityBridge.cs','HostIoBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostCapacityProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostCapacityProbe')
        project=consumer/'HostCapacityProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostCapacityProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostCapacityProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostCapacityProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-capacity/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native statvfs layout/invariants and raw/optimized JIT/AOT private filesystem capacity pass; HostFiles and InstanceIo regressions pass')
finally:save()
