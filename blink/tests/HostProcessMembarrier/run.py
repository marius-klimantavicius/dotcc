...
def main():
    base = BLINK / "artifacts/host-process-membarrier"
    ...
    try:
        ...
        native = work / "native"
        run("native-build", [cc, "-std=c17", "-Wall", "-Wextra", "-Werror", "-pthread", inputs / "probe.c", "-o", native])
        ...
        for label, generated in (("raw", raw), ("optimized", optimized)):
            ...
            for key, value in (("TargetFramework", "net10.0"), ("OutputType", "Exe"),
                               ...
                               ("AssemblyName", "HostProcessMembarrierProbe"), ("WarningsAsErrors", "CS8500")):
            ...
            ET.SubElement(items, "TrimmerRootAssembly", Include="HostProcessMembarrierProbe")
            project = consumer / "HostProcessMembarrierProbe.csproj"
            ...
            dll = consumer / "bin/Release/net10.0/HostProcessMembarrierProbe.dll"
            ...
            executable = publish / "HostProcessMembarrierProbe"
