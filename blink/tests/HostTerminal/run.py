#!/usr/bin/env python3
"""Run nonterminal descriptor and termios storage behavior through native and authored BCL callbacks."""
...
base=ROOT/'generated/host-terminal';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-terminal'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'real authored nonterminal and termios storage callbacks, not core execution','passed':False,'results':{}}
...
try:
    ...
    run(['cc','-std=c17','-Wall','-Wextra','-Werror','-c',a/'native-descriptor.c','-o',a/'native-descriptor.o'],'native-stub-build')
    run(['cc','-std=c17','-Wall','-Wextra','-Werror','-I',a/'profile',a/'probe.c',a/'HostTerminal.c',a/'HostTermios.c',a/'native-descriptor.o','-o',a/'native-staged'],'native-staged-build')
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_TERMINAL','-I',a/'profile',a/'probe.c',a/'HostTerminal.c',a/'HostTermios.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostIoBridge.cs','HostTerminalBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostTerminalProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostTerminalProbe')
        project=consumer/'HostTerminalProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostTerminalProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostTerminalProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostTerminalProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-terminal/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native libc, native staged and raw/optimized JIT/AOT terminal policies and full termios storage comparison pass')
finally:save()
