# Historical CPU evidence before normal-only selection

This preserved record describes earlier completed diagnostics, including custom
fault cases and the former INT3 hardware witness. Those executions are outside
the current selection; commands and default-run descriptions below are archival.
No historical receipt is rewritten or counted as a fresh normal-only run.
