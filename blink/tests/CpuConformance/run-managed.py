...
try:
    ...
    native_output=run(['python3',ROOT/'tests/CpuConformance/run.py'],'native-corpus')
    native_receipt=Path(native_output.strip().rsplit('receipt ',1)[1]);native=json.loads(native_receipt.read_text())
    if not native['passed']:raise RuntimeError('native corpus incomplete')
    ...
    for name in ['managed-driver.c','interpreter.c','corpus.h','output.h','Program.cs']:shutil.copyfile(ROOT/'tests/CpuConformance'/name,a/'source'/name)
    ...
    for label in ['raw','optimized']:
        ...
        for mode in ['jit','aot']:
            ...
            for case in cases:
                ...
                r['comparisons'].append({'mode':label+'-'+mode,'case':case['name'],'matched':not differences,'differences':differences,'owner':owner});save()
                if differences:raise RuntimeError('CPU mismatch '+case['name']+' '+str(differences))
    ...
    r['passed']=True;save();print('48 actual translated-core CPU comparisons pass; receipt '+str(out/'receipt.json'))
finally:save()
