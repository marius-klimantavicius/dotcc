#!/usr/bin/env python3
"""Run read-only clock C behavior through native and authored BCL callbacks."""
...
base=ROOT/'generated/host-clocks';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-clocks'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'real authored read-only clock callbacks, not core execution','passed':False,'results':{}}
...
try:
    for name in ['probe.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostClocks'/name,a/name)
    shutil.copyfile(ROOT/'src/HostEnvironment/HostEnvironmentBridge.cs',a/'HostEnvironmentBridge.cs')
    shutil.copyfile(ROOT/'src/HostEnvironment/HostClockBridge.cs',a/'HostClockBridge.cs')
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_CLOCKS','-I',a/'profile',a/'probe.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostEnvironmentBridge.cs','HostClockBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostClocksProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostClocksProbe')
        project=consumer/'HostClocksProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostClocksProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostClocksProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostClocksProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-clocks/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native and raw/optimized JIT/AOT read-only clocks pass; provider resolution, precision, errors and worker isolation verified')
finally:save()
