#!/usr/bin/env python3
"""Run private atomic namespace changes through native invariants and authored C callbacks."""
...
base=ROOT/'generated/host-namespace';base.mkdir(parents=True,exist_ok=True)
a=Path(tempfile.mkdtemp(prefix='attempt-',dir=base))
out=ROOT/'artifacts/host-namespace'/a.name;out.mkdir(parents=True)
cli=REPO/'DotCC/bin/Release/net10.0/dotcc.dll';post=REPO/'DotCC.PostProcess/bin/Release/net10.0/dotcc-postprocess.dll'
r={'scope':'private atomic namespace changes callbacks, not full core or host filesystem' ,'passed':False,'results':{}}
...
try:
    for name in ['probe.c','Program.cs']:shutil.copyfile(ROOT/'tests/HostNamespace'/name,a/name)
    ...
    run(['dotnet',cli,'-std=c17','-DBLINK_MANAGED_NAMESPACE','-I',a/'profile',a/'probe.c','--runtime=c','--emit=managedlib','--nest-types','--class-name','Blink','--namespace','Managed.Emulation','-o',raw],'translate')
    ...
    for label,generated in [('raw',raw),('optimized',opt)]:
        ...
        for name in ['Program.cs','HostNamespaceBridge.cs','HostFileMetadataBridge.cs','HostIoBridge.cs','HostPathsBridge.cs']:shutil.copyfile(a/name,consumer/name)
        xml=ET.Element('Project',Sdk='Microsoft.NET.Sdk');props=ET.SubElement(xml,'PropertyGroup')
        for key,value in [('TargetFramework','net10.0'),('OutputType','Exe'),('AllowUnsafeBlocks','true'),('Nullable','enable'),('AssemblyName','HostNamespaceProbe'),('WarningsAsErrors','CS8500')]:ET.SubElement(props,key).text=value
        ...
        ET.SubElement(items,'TrimmerRootAssembly',Include='HostNamespaceProbe')
        project=consumer/'HostNamespaceProbe.csproj';ET.ElementTree(xml).write(project,encoding='unicode')
        if label=='optimized':
        ...
        actual=run(['dotnet',consumer/'bin/Release/net10.0/HostNamespaceProbe.dll'],label+'-jit',30)
        ...
        actual=run([publish/'HostNamespaceProbe'],label+'-aot',30)
        if actual!=expected:raise RuntimeError(label+' AOT differs from native')
        r['results'][label+'-aot']['binarySha256']=sha(publish/'HostNamespaceProbe')
    ...
    r['passed']=True;save()
    (ROOT/'artifacts/host-namespace/latest.json').write_text(json.dumps({'receipt':str((out/'receipt.json').relative_to(ROOT))},indent=2)+'\n')
    print('native namespace semantics and raw/optimized JIT/AOT private atomic namespace changes pass; HostFiles and InstanceIo regressions pass')
finally:save()
