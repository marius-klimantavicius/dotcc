#!/usr/bin/env python3
"""Qualify NativeAOT async HTTP through the public machine API in both modes."""
import argparse
import hashlib
import json
import os
...
BLINK = ROOT / "blink"
PROJECT = BLINK / "tests/NonblockingGuest/NonblockingGuest.csproj"
...
def main():
    ...
    parser.add_argument("--elf", type=Path, default=BLINK / "build/httpclient-nativeaot/HttpClientGuest")
    ...
    artifacts = BLINK / "artifacts/nonblocking-guest"
    ...
    jit = PROJECT.parent / "bin/Release/net10.0/NonblockingGuest.dll"
    aot_dir = BLINK / "build/nonblocking-guest-aot"
    aot = aot_dir / "NonblockingGuest"
    ...
    try:
        ...
        for folder in (BLINK / "tests/HttpClientGuest", PROJECT.parent):
        ...
        run("native", ["dotnet", jit, elf, "native", output / "native"], timeout=300)
