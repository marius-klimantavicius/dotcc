...
def run(command,log,timeout=900,env=None):
    log.parent.mkdir(parents=True,exist_ok=True);started=time.monotonic()
    with log.open('w') as output:
        try:code=subprocess.run(list(map(str,command)),cwd=REPO,stdout=output,stderr=subprocess.STDOUT,timeout=timeout,env=env).returncode
        except subprocess.TimeoutExpired:code=124
    print(f'{log.name}: exit {code}',flush=True)
    return {'command':list(map(str,command)),'exit':code,'seconds':round(time.monotonic()-started,3),'log':str(log.relative_to(ROOT)),'sha256':sha(log)}
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('action',choices=['translate','run','all'],default='all',nargs='?');p.add_argument('--profile',choices=['release','debug'],default='release');p.add_argument('--form',choices=['all','raw','optimized'],default='all');p.add_argument('--mode',choices=['all','jit','aot'],default='all');p.add_argument('--case',action='append',dest='selected');args=p.parse_args()
    ...
    state=artifacts/'translation.json';receipt={'platform':platform.platform(),'profile':args.profile,'runs':[]}
    configs=[f for f in (ROOT/'config').rglob('*') if f.is_file()];authored=[ROOT/'src/native-adapter.c',ROOT/'tests/native-main.c',ROOT/'tests/native-cases.inc',pathlib.Path(__file__)]
    ...
    if args.action in ('translate','all'):
        ...
        with tempfile.TemporaryDirectory(prefix='emit-',dir=dest) as temp:
            ...
            command=['dotnet',COMPILER,'-std=c17','-DPINTA_HAVE_LONG_LITERAL=1','-DPINTA_DEBUG='+str(int(args.profile=='debug')),'-I',ROOT/'config','-I',ROOT/'tests','-I',ROOT,'-I',source/'Marius.Pinta/inc','-I',source/'Marius.Pinta/tests',*units,'--emit=csproj','--runtime=c','--split=size','--split-size=102400','-o',emitted]
    ...
    env=dict(os.environ,PINTA_FIXTURES=str(ROOT/'ref/upstream/Marius.Pinta.Test.Files'))
    failed=False
    try:
        for form in forms:
            ...
            for mode in modes:
                ...
                for case in cases:
                    log=artifacts/f'{form}-{mode}-{case}.log';result=run([*invoke,'--case',case],log,60,env)
