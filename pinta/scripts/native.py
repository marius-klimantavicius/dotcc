...
def main():
    p=argparse.ArgumentParser();p.add_argument('--profile',choices=['release','debug'],default='release');p.add_argument('--stage',choices=['pristine','corrected'],default='pristine');p.add_argument('--build-only',action='store_true');p.add_argument('--cases',action='store_true');args=p.parse_args()
    ...
    if not args.build_only:
        ...
        for group,arguments in runs:
            ...
            try:r=subprocess.run([str(out/'tests'),*arguments],env=env,capture_output=True,text=True,timeout=60);status=r.returncode;log=r.stdout+r.stderr
            ...
            (artifacts/(group+'.log')).write_text(log);receipt['cases'].append({'group':group,'exit':status,'seconds':round(time.monotonic()-started,3)})
    ...
    return int(receipt['abi_exit']!=0 or any(c['exit'] for c in receipt['cases']))
