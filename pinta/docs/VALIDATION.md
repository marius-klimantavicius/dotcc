...
| Corrected native release/debug | All 16 groups and 73 isolated cases pass, with 2,034 assertions per isolated corpus. The globals-properties failure was an invalid pointer-subtraction calculation in GC scratch-buffer sizing; byte-based sizing fixes it. After the heap-allocation guards, the earlier creation sweep completed: 3,474 successful initializations and 623 rejected arena sizes, with canaries intact. Misaligned tiny-arena and stack-length-overflow checks also passed. String/character/weak allocation probes each retained 41 rooted integers, then returned OOM status 4 with a null result. The native callback result survived confirmed object and payload relocation, with one open, 43 three-byte reads and one close. |
...
| Actual-core translation | All 29 corrected core units emit and pass semantic postprocessing. Raw and optimized libraries build without warnings/errors. Reduced compiler fixes cover typedef/member names, pasted wide literals, C identifiers named `null`, and wide-character conversion. The GC scratch-buffer fix resolves the native globals-properties discrepancy. |
...
| Translated upstream tests | All four Linux forms in each of the release and diagnostic profiles pass 73 of 73 original test bodies and 2,034 assertions each. After the GC scratch-buffer fix, all 73 cases match corrected native output after removing timing/stream-order differences. Detailed globals-properties traces also match native in all eight forms. Both matrix runners return exit 0. |
...
edge cases and three unqualified nonempty fixtures.
Additional malformed-module/debugger investigation remains paused by an
