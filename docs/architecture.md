...
- `Compiler.SourceFiles` — project source layout from backend function boundaries, with partial classes, UTF-8 size grouping and a generated-file manifest. `EmitCSharpFiles`/`LinkObjectFiles` return named sources without Roslyn; `WriteCSharpFiles` writes them and removes obsolete generated files.
