...
| top-level / global `const`/`var` | ✅ | a runtime global → a field of the generated globals struct held by the function owner's `[FixedAddressValueType] static Globals` field (the same path the C front-end's file-scope variables take). Typed keeps its annotation; untyped infers from the initializer (`const N = 5;` → `int`). The owner static constructor initializes globals in source order, so a global may reference an EARLIER global. `const`-ness isn't enforced (both lower to mutable storage — observably identical for a correct Zig program). An aggregate (struct), `[N]T` array, and `undefined` global are supported (Milestone K — an array routes through a pinned, program-lifetime backing store). A **fn-pointer global** works — typed (`const h: *const fn (i32) i32 = &fn;`) or INFERRED (`const alias = &fn;`, where `&fn` is a callable `CType.Func` value) — and may FORWARD-reference a function declared later (functions are registered before globals). **Cut:** a forward reference to a later global CONST VALUE (`const a = b + 1; const b = 41;`) — order-independent comptime-const resolution isn't modeled; declare in dependency order |
| `threadlocal var x: T = 0;` | ✅ | thread storage duration on a container-level `var` → a field of a generated thread-globals struct backed by a pinned one-element array held in one `[ThreadStatic]` reference (the C `_Thread_local` twofer — same `Symbol.IsThreadLocal` marker and backend emit). Every thread gets its own zero-initialized, address-stable slot. **V1 constraints:** zero initializer only, scalar only, typed form only, and container level only — a function-local `threadlocal` is rejected exactly as real zig rejects it. Oracle `threadlocal_var` (=42 vs real zig 0.17) |
