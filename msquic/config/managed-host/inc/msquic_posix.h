...
/* Managed host event queues are opaque through the public API. Their complete
   CxPlat layout lives in the explicit managed platform header. External worker
   APIs remain unqualified until the host supports their lifecycle contract. */
typedef struct CXPLAT_EVENTQ QUIC_EVENTQ;
typedef struct CXPLAT_CQE QUIC_CQE;
typedef struct CXPLAT_SQE QUIC_SQE;
typedef void QUIC_EVENT_COMPLETION(QUIC_CQE* Cqe);
