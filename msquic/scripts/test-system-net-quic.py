#!/usr/bin/env python3
"""Build and exercise the direct System.Net.Quic facade, serially in JIT/NativeAOT."""
import argparse
import os
...
logs = ROOT / 'artifacts/system-net-quic'
logs.mkdir(parents=True, exist_ok=True)
project = ROOT / 'tests/SystemNetQuic/SystemNetQuic.csproj'
...
commands = [('jit', ['dotnet', project.parent / 'bin/Release/net10.0/SystemNetQuic.dll'])]
if not args.jit_only:
    output = ROOT / 'build/system-net-quic/aot'
    run(['dotnet', 'publish', project, '-c', 'Release', '-r', 'linux-x64',
         '-p:PublishAot=true', '-o', output, '--nologo'], 'aot-build')
    commands.append(('aot', [output / 'SystemNetQuic']))
expected = None
    ...
    for ipv6 in (False, True):
        for no_cache in (False, True):
            ...
            try:
                ...
                keylog.unlink(missing_ok=True)
print(f'PASS System.Net.Quic: 23 cases x {len(commands)} runtimes x IPv4/IPv6 x cached/uncached; TLS secrets checked')
