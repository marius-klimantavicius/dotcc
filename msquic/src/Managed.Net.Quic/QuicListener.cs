...
using NEW_CONNECTION_DATA = Managed.Transport.MsQuic.QUIC_LISTENER_EVENT_NEW_CONNECTION_DATA;
using STOP_COMPLETE_DATA = Managed.Transport.MsQuic.QUIC_LISTENER_EVENT_STOP_COMPLETE_DATA;
...
public sealed partial class QuicListener : IAsyncDisposable
{
    ...
    private unsafe uint HandleEventNewConnection(ref NEW_CONNECTION_DATA data)
    {
    ...
    private uint HandleEventStopComplete()
    {
        _shutdownTcs.TrySetResult();
        return QUIC_STATUS_SUCCESS;
    }

    private uint HandleListenerEvent(ref QUIC_LISTENER_EVENT listenerEvent)
        => listenerEvent.Type switch
        {
            QUIC_LISTENER_EVENT_TYPE.QUIC_LISTENER_EVENT_NEW_CONNECTION => HandleEventNewConnection(ref listenerEvent.NEW_CONNECTION),
            QUIC_LISTENER_EVENT_TYPE.QUIC_LISTENER_EVENT_STOP_COMPLETE => HandleEventStopComplete(),
            _ => QUIC_STATUS_SUCCESS
        };

    private static unsafe uint NativeCallback(QUIC_HANDLE* listener, void* context, QUIC_LISTENER_EVENT* listenerEvent)
    {
