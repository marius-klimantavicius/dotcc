...
using CONNECTED_DATA = Managed.Transport.MsQuic.QUIC_CONNECTION_EVENT_CONNECTED_DATA;
using LOCAL_ADDRESS_CHANGED_DATA = Managed.Transport.MsQuic.QUIC_CONNECTION_EVENT_LOCAL_ADDRESS_CHANGED_DATA;
using PEER_ADDRESS_CHANGED_DATA = Managed.Transport.MsQuic.QUIC_CONNECTION_EVENT_PEER_ADDRESS_CHANGED_DATA;
using PEER_CERTIFICATE_RECEIVED_DATA = Managed.Transport.MsQuic.QUIC_CONNECTION_EVENT_PEER_CERTIFICATE_RECEIVED_DATA;
using PEER_STREAM_STARTED_DATA = Managed.Transport.MsQuic.QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED_DATA;
using STREAMS_AVAILABLE_DATA = Managed.Transport.MsQuic.QUIC_CONNECTION_EVENT_STREAMS_AVAILABLE_DATA;
using SHUTDOWN_COMPLETE_DATA = Managed.Transport.MsQuic.QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE_DATA;
using SHUTDOWN_INITIATED_BY_PEER_DATA = Managed.Transport.MsQuic.QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_PEER_DATA;
using SHUTDOWN_INITIATED_BY_TRANSPORT_DATA = Managed.Transport.MsQuic.QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_TRANSPORT_DATA;
using QuicAddr = Managed.Transport.MsQuic.QUIC_ADDR;
...
/// or create with a static method <see cref="QuicConnection.ConnectAsync(Managed.Net.Quic.QuicClientConnectionOptions, CancellationToken)" /> (outbound connection).
///
...
public sealed partial class QuicConnection : IAsyncDisposable
{
    ...
    /// </summary>
    /// <remarks>
    /// This implementation uses translated MsQuic and PicoTLS with the .NET platform host.
    /// </remarks>
    ...
    public static ValueTask<QuicConnection> ConnectAsync(QuicClientConnectionOptions options, CancellationToken cancellationToken = default)
    {
        if (!IsSupported)
        {
            throw new PlatformNotSupportedException(SR.Format(SR.ManagedNetQuic_PlatformNotSupported, MsQuicApi.NotSupportedReason ?? "General loading failure."));
        }
    ...
    internal unsafe QuicConnection(QUIC_HANDLE* handle, QUIC_NEW_CONNECTION_INFO* info)
    {
        ...
        {
            _handle = new MsQuicContextSafeHandle(handle, context, SafeHandleType.Connection);
            delegate* <QUIC_HANDLE*, void*, QUIC_CONNECTION_EVENT*, uint> nativeCallback = &NativeCallback;
    ...
    private async ValueTask FinishConnectAsync(QuicClientConnectionOptions options, CancellationToken cancellationToken = default)
    {
        if (_connectedTcs.TryInitialize(out ValueTask valueTask, this, cancellationToken))
        {
            ...
            // Normalize IDN before handing UTF-8 bytes to translated MsQuic/PicoTLS.
            ...
            unsafe
            {
                byte* targetHostPtr = !string.IsNullOrEmpty(sni) ? Utf8StringMarshaller.ConvertToUnmanaged(TargetHostNameHelper.NormalizeHostName(sni)) : null;
                try
                {
    ...
    private unsafe uint HandleEventConnected(ref CONNECTED_DATA data)
    {
    ...
    private uint HandleEventShutdownInitiatedByTransport(ref SHUTDOWN_INITIATED_BY_TRANSPORT_DATA data)
    {
    ...
    private uint HandleEventShutdownInitiatedByPeer(ref SHUTDOWN_INITIATED_BY_PEER_DATA data)
    {
    ...
    private unsafe uint HandleEventLocalAddressChanged(ref LOCAL_ADDRESS_CHANGED_DATA data)
    {
        _localEndPoint = MsQuicHelpers.QuicAddrToIPEndPoint(data.Address);
        return QUIC_STATUS_SUCCESS;
    }
    private unsafe uint HandleEventPeerAddressChanged(ref PEER_ADDRESS_CHANGED_DATA data)
    {
        _remoteEndPoint = MsQuicHelpers.QuicAddrToIPEndPoint(data.Address);
        return QUIC_STATUS_SUCCESS;
    }
    private unsafe uint HandleEventPeerStreamStarted(ref PEER_STREAM_STARTED_DATA data)
    {
        QuicStream stream = new QuicStream(_handle, data.Stream, data.Flags, _defaultStreamErrorCode);

        if (NetEventSource.Log.IsEnabled())
        {
            QuicStreamType type = data.Flags.HasFlag(QUIC_STREAM_OPEN_FLAGS.QUIC_STREAM_OPEN_FLAG_UNIDIRECTIONAL) ? QuicStreamType.Unidirectional : QuicStreamType.Bidirectional;
    ...
    private uint HandleEventStreamsAvailable(ref STREAMS_AVAILABLE_DATA data)
    {
    ...
    private unsafe uint HandleEventPeerCertificateReceived(ref PEER_CERTIFICATE_RECEIVED_DATA data)
    {
        //
    ...
    private uint HandleConnectionEvent(ref QUIC_CONNECTION_EVENT connectionEvent)
        => connectionEvent.Type switch
        {
            QUIC_CONNECTION_EVENT_TYPE.QUIC_CONNECTION_EVENT_CONNECTED => HandleEventConnected(ref connectionEvent.CONNECTED),
            QUIC_CONNECTION_EVENT_TYPE.QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_TRANSPORT => HandleEventShutdownInitiatedByTransport(ref connectionEvent.SHUTDOWN_INITIATED_BY_TRANSPORT),
            QUIC_CONNECTION_EVENT_TYPE.QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_PEER => HandleEventShutdownInitiatedByPeer(ref connectionEvent.SHUTDOWN_INITIATED_BY_PEER),
            QUIC_CONNECTION_EVENT_TYPE.QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE => HandleEventShutdownComplete(),
            QUIC_CONNECTION_EVENT_TYPE.QUIC_CONNECTION_EVENT_LOCAL_ADDRESS_CHANGED => HandleEventLocalAddressChanged(ref connectionEvent.LOCAL_ADDRESS_CHANGED),
            QUIC_CONNECTION_EVENT_TYPE.QUIC_CONNECTION_EVENT_PEER_ADDRESS_CHANGED => HandleEventPeerAddressChanged(ref connectionEvent.PEER_ADDRESS_CHANGED),
            QUIC_CONNECTION_EVENT_TYPE.QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED => HandleEventPeerStreamStarted(ref connectionEvent.PEER_STREAM_STARTED),
            QUIC_CONNECTION_EVENT_TYPE.QUIC_CONNECTION_EVENT_STREAMS_AVAILABLE => HandleEventStreamsAvailable(ref connectionEvent.STREAMS_AVAILABLE),
            QUIC_CONNECTION_EVENT_TYPE.QUIC_CONNECTION_EVENT_PEER_CERTIFICATE_RECEIVED => HandleEventPeerCertificateReceived(ref connectionEvent.PEER_CERTIFICATE_RECEIVED),
            _ => QUIC_STATUS_SUCCESS,
        };

    private static unsafe uint NativeCallback(QUIC_HANDLE* connection, void* context, QUIC_CONNECTION_EVENT* connectionEvent)
    {
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        {
            if (!_remoteCertificateExposed) _remoteCertificate?.Dispose();
        }
