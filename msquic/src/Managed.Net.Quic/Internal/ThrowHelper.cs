...
internal static class ThrowHelper
{
    ...
    internal static bool TryGetStreamExceptionForMsQuicStatus(uint status, [NotNullWhen(true)] out Exception? exception)
    {
        ...
        else if (MsQuicApi.StatusFailed(status))
        {
    ...
    internal static Exception GetExceptionForMsQuicStatus(uint status, long? errorCode = default, string? message = null)
    {
        ...
        if (status != 0)
        {
            ...
            ex.HResult = unchecked((int)status);
        }

        return ex;

        static Exception GetExceptionInternal(uint status, long? errorCode, string? message)
        {
            //
    ...
    internal static void ThrowIfMsQuicError(uint status, string? message = null)
    {
    ...
    internal static void ThrowMsQuicException(uint status, string? message = null)
    {
        throw GetExceptionForMsQuicStatus(status, message: message);
    }

    internal static string GetErrorMessageForStatus(uint status, string? message)
    {
        return (message ?? "Status code") + ": " + GetErrorMessageForStatus(status);
    }

    internal static string GetErrorMessageForStatus(uint status)
    {
