...
using static Managed.Net.Quic.MsQuicHelpers;
using static Managed.Transport.MsQuic;
using PEER_RECEIVE_ABORTED_DATA = Managed.Transport.MsQuic.QUIC_STREAM_EVENT_PEER_RECEIVE_ABORTED_DATA;
using PEER_SEND_ABORTED_DATA = Managed.Transport.MsQuic.QUIC_STREAM_EVENT_PEER_SEND_ABORTED_DATA;
using RECEIVE_DATA = Managed.Transport.MsQuic.QUIC_STREAM_EVENT_RECEIVE_DATA;
using SEND_COMPLETE_DATA = Managed.Transport.MsQuic.QUIC_STREAM_EVENT_SEND_COMPLETE_DATA;
using SEND_SHUTDOWN_COMPLETE_DATA = Managed.Transport.MsQuic.QUIC_STREAM_EVENT_SEND_SHUTDOWN_COMPLETE_DATA;
using SHUTDOWN_COMPLETE_DATA = Managed.Transport.MsQuic.QUIC_STREAM_EVENT_SHUTDOWN_COMPLETE_DATA;
using START_COMPLETE_DATA = Managed.Transport.MsQuic.QUIC_STREAM_EVENT_START_COMPLETE_DATA;
...
public sealed partial class QuicStream
{
    ...
    internal unsafe QuicStream(MsQuicContextSafeHandle connectionHandle, QUIC_HANDLE* handle, QUIC_STREAM_OPEN_FLAGS flags, long defaultErrorCode)
    {
        ...
        try
        {
            ...
            delegate* <QUIC_HANDLE*, void*, QUIC_STREAM_EVENT*, uint> nativeCallback = &NativeCallback;
    ...
    internal ValueTask StartAsync(Action<QuicStreamType> decrementStreamCapacity, CancellationToken cancellationToken = default)
    {
        ...
        unsafe
        {
            var status = MsQuicApi.Api.StreamStart(
                _handle,
    ...
    public ValueTask WriteAsync(ReadOnlyMemory<byte> buffer, bool completeWrites, CancellationToken cancellationToken = default)
    {
        ...
        if (Interlocked.CompareExchange(ref _sendLocked, 1, 0) == 0)
        {
            unsafe
            {
                ...
                var status = MsQuicApi.Api.StreamSend(
                    _handle,
    ...
    private uint HandleEventStartComplete(ref START_COMPLETE_DATA data)
    {
        Debug.Assert(_decrementStreamCapacity is not null);

        _id = unchecked((long)data.ID);
        if (MsQuicApi.StatusSucceeded(data.Status))
        {
    ...
    private unsafe uint HandleEventReceive(ref RECEIVE_DATA data)
    {
    ...
    private uint HandleEventSendComplete(ref SEND_COMPLETE_DATA data)
    {
    ...
    private uint HandleEventPeerSendAborted(ref PEER_SEND_ABORTED_DATA data)
    {
        _receiveTcs.TrySetException(ThrowHelper.GetStreamAbortedException((long)data.ErrorCode));
        return QUIC_STATUS_SUCCESS;
    }
    private uint HandleEventPeerReceiveAborted(ref PEER_RECEIVE_ABORTED_DATA data)
    {
        _sendTcs.TrySetException(ThrowHelper.GetStreamAbortedException((long)data.ErrorCode));
        return QUIC_STATUS_SUCCESS;
    }
    private uint HandleEventSendShutdownComplete(ref SEND_SHUTDOWN_COMPLETE_DATA data)
    {
        if (data.Graceful != 0)
        {
            _sendTcs.TrySetResult(final: true);
    ...
    private uint HandleEventShutdownComplete(ref SHUTDOWN_COMPLETE_DATA data)
    {
    ...
    private uint HandleEventPeerAccepted()
    {
        _startedTcs.TrySetResult();
        return QUIC_STATUS_SUCCESS;
    }

    private uint HandleStreamEvent(ref QUIC_STREAM_EVENT streamEvent)
        => streamEvent.Type switch
        {
            QUIC_STREAM_EVENT_TYPE.QUIC_STREAM_EVENT_START_COMPLETE => HandleEventStartComplete(ref streamEvent.START_COMPLETE),
            QUIC_STREAM_EVENT_TYPE.QUIC_STREAM_EVENT_RECEIVE => HandleEventReceive(ref streamEvent.RECEIVE),
            QUIC_STREAM_EVENT_TYPE.QUIC_STREAM_EVENT_SEND_COMPLETE => HandleEventSendComplete(ref streamEvent.SEND_COMPLETE),
            QUIC_STREAM_EVENT_TYPE.QUIC_STREAM_EVENT_PEER_SEND_SHUTDOWN => HandleEventPeerSendShutdown(),
            QUIC_STREAM_EVENT_TYPE.QUIC_STREAM_EVENT_PEER_SEND_ABORTED => HandleEventPeerSendAborted(ref streamEvent.PEER_SEND_ABORTED),
            QUIC_STREAM_EVENT_TYPE.QUIC_STREAM_EVENT_PEER_RECEIVE_ABORTED => HandleEventPeerReceiveAborted(ref streamEvent.PEER_RECEIVE_ABORTED),
            QUIC_STREAM_EVENT_TYPE.QUIC_STREAM_EVENT_SEND_SHUTDOWN_COMPLETE => HandleEventSendShutdownComplete(ref streamEvent.SEND_SHUTDOWN_COMPLETE),
            QUIC_STREAM_EVENT_TYPE.QUIC_STREAM_EVENT_SHUTDOWN_COMPLETE => HandleEventShutdownComplete(ref streamEvent.SHUTDOWN_COMPLETE),
            QUIC_STREAM_EVENT_TYPE.QUIC_STREAM_EVENT_PEER_ACCEPTED => HandleEventPeerAccepted(),
            _ => QUIC_STATUS_SUCCESS
        };

    private static unsafe uint NativeCallback(QUIC_HANDLE* stream, void* context, QUIC_STREAM_EVENT* streamEvent)
    {
    ...
    public override async ValueTask DisposeAsync()
    {
        ...
        {
            var status = MsQuicApi.Api.StreamShutdown(
                _handle,
                flags,
                (ulong)errorCode);
            if (MsQuicApi.StatusFailed(status))
