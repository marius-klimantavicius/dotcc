...
public sealed unsafe partial class MsQuicHost
{
    private sealed class DatagramSend : IDisposable
    {
        ...
        private int submitted, disposed;
        internal bool Submitted => Volatile.Read(ref submitted) != 0;
        internal DatagramSend(DatagramSocket socket, ushort maximum) { Socket = socket; Maximum = maximum; }
        internal QUIC_BUFFER* Allocate(ushort length)
        {
            if (Buffer != null || length == 0 || length > Maximum || Volatile.Read(ref submitted) != 0) return null;
            ...
            header->Length = length; Payload = payload; Buffer = header;
            return header;
        }
        internal void FreeBuffer(QUIC_BUFFER* buffer)
        {
            if (buffer == null || buffer != Buffer || Volatile.Read(ref submitted) != 0) FatalInvariant("Invalid send buffer release.");
            Socket.Path.Host.FreePlatformMemory(Buffer, DatagramAllocationTag); Buffer = null; Payload = null;
        }
        internal void Submit(CXPLAT_ROUTE* route)
        {
            if (Interlocked.Exchange(ref submitted, 1) != 0 || Buffer == null || Payload == null || Buffer->Length > Payload.Length)
            ...
            try
            {
                ...
                Socket.BeginSend(); admitted = true;
            ...
            {
                if (admitted) Complete(error);
                else
                {
                    Interlocked.Increment(ref Socket.Path.Host.datagramSendErrors);
        ...
        private void Complete(Exception? error)
        {
            ...
            catch (Exception failure) { FatalInvariant("UDP send completion failed: " + failure); }
            finally { Socket.EndSend(); }
        }
        public void Dispose()
        {
            if (Interlocked.Exchange(ref disposed, 1) != 0) return;
            if (Buffer != null) Socket.Path.Host.FreePlatformMemory(Buffer, DatagramAllocationTag);
            Buffer = null; Payload = null;
        }
    }
    private static CXPLAT_SEND_DATA* DatapathSendAlloc(void* context, CXPLAT_SOCKET* socket, CXPLAT_SEND_CONFIG* config)
    {
        if (config == null || config->ECN != 0 || config->DSCP != 0 || (config->Flags & ~(byte)CXPLAT_SEND_FLAGS.CXPLAT_SEND_FLAGS_MAX_THROUGHPUT) != 0 || config->MaxPacketSize > DatagramMaxPayload) return null;
        try
        {
        ...
        catch (Exception error) { FatalInvariant(error.ToString()); return null; }
    }
    private static QUIC_BUFFER* DatapathSendBuffer(void* context, CXPLAT_SEND_DATA* send, ushort length)
    { try { return FromContext(context).Resource<DatagramSend>(send).Allocate(length); } catch (OutOfMemoryException) { return null; } catch (Exception error) { FatalInvariant(error.ToString()); return null; } }
    private static void DatapathSendFreeBuffer(void* context, CXPLAT_SEND_DATA* send, QUIC_BUFFER* buffer)
    { try { FromContext(context).Resource<DatagramSend>(send).FreeBuffer(buffer); } catch (Exception error) { FatalInvariant(error.ToString()); } }
    private static void DatapathSendFree(void* context, CXPLAT_SEND_DATA* send)
    { try { var host = FromContext(context); if (host.Resource<DatagramSend>(send).Submitted) FatalInvariant("Caller freed an in-flight send."); host.ReleaseResource<DatagramSend>(send); } catch (Exception error) { FatalInvariant(error.ToString()); } }
    private static byte DatapathSendFull(void* context, CXPLAT_SEND_DATA* send)
    { try { return FromContext(context).Resource<DatagramSend>(send).Buffer == null ? (byte)0 : (byte)1; } catch (Exception error) { FatalInvariant(error.ToString()); return 1; } }
    private static void DatapathSocketSend(void* context, CXPLAT_SOCKET* socket, CXPLAT_ROUTE* route, CXPLAT_SEND_DATA* send)
    {
        try
        {
            var host = FromContext(context); var item = host.Resource<DatagramSend>(send);
            if (route == null || !ReferenceEquals(item.Socket, host.Resource<DatagramSocket>(socket))) FatalInvariant("Send submitted to a foreign socket or invalid route.");
            item.Submit(route);
        }
...
internal static partial class DatagramAsync
{
    internal static void Send(Socket socket, ReadOnlyMemory<byte> payload, IPEndPoint remote, Action<Exception?> complete)
    {
        ...
        try { operation = socket.SendToAsync(payload, SocketFlags.None, remote); }
        catch (Exception error) { complete(error); return; }
        ...
        else _ = Await(operation, payload.Length, complete);
    }
    ...
    private static async Task Await(ValueTask<int> operation, int expected, Action<Exception?> complete)
    {
        ...
        try { if (await operation.ConfigureAwait(false) != expected) failure = new SocketException((int)SocketError.MessageSize); }
