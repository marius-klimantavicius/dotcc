...
public sealed unsafe partial class MsQuicHost
{
    ...
    private readonly Dictionary<nuint, PlatformAllocation> _platformAllocations = new();
    ...
    internal int OutstandingPlatformAllocations { get { lock (_allocationGate) return _platformAllocations.Count; } }
    ...
    internal void* AllocatePlatformMemory(ulong size, uint tag, bool zero = true)
    {
        ...
        if (size > (ulong)Array.MaxLength - 15) return null;
        try
        {
            var bytes = zero ? GC.AllocateArray<byte>((int)size + 15, pinned: true)
                : GC.AllocateUninitializedArray<byte>((int)size + 15, pinned: true);
            fixed (byte* start = bytes)
            {
                var address = ((nuint)start + 15) & ~(nuint)15;
                lock (_allocationGate) _platformAllocations.Add(address, new PlatformAllocation(bytes, size, tag));
                return (void*)address;
            }
        }
    ...
    private static void PlatformSleep(void* context, uint milliseconds)
    {
        ...
        while (remaining > int.MaxValue) { Thread.Sleep(int.MaxValue); remaining -= int.MaxValue; }
        Thread.Sleep((int)remaining);
    }
    private static uint PlatformRandom(void* context, uint length, void* buffer)
    {
        if (length > int.MaxValue || (length != 0 && buffer == null)) return Status.InvalidParameter;
        try { RandomNumberGenerator.Fill(new Span<byte>(buffer, (int)length)); return Status.Success; }
        catch (CryptographicException) { return Status.InternalError; }
    }
    private static void PlatformLoad(void* context)
    {
        if (Interlocked.CompareExchange(ref FromContext(context)._platformLoaded, 1, 0) != 0) FatalInvariant("Platform already loaded");
    }
    ...
    {
        if (result == null) return Status.InvalidParameter;
        *result = null;
    ...
    {
        if (storage != null) FatalInvariant("Persistent storage is not supported by this host");
    }
    private static string PlatformText(byte* text) => text == null ? "<null>" : Marshal.PtrToStringUTF8((nint)text) ?? "";
    private static void PlatformLogAssert(void* context, byte* file, int line, byte* expression)
        => Console.Error.WriteLine($"MsQuic assertion {PlatformText(file)}:{line}: {PlatformText(expression)}");
    private static void PlatformBugcheck(void* context, byte* file, int line, byte* expression)
        => FatalInvariant($"MsQuic assertion {PlatformText(file)}:{line}: {PlatformText(expression)}");
    ...
    private sealed class PlatformLock : IDisposable
    {
        ...
        internal void Enter()
        {
            ...
            Monitor.Enter(_gate);
            if (_disposed) FatalInvariant("Acquire disposed recursive lock");
        }
        internal void Exit()
        {
            if (!Monitor.IsEntered(_gate)) FatalInvariant("Release recursive lock from non-owner");
        ...
        public void Dispose()
        {
            lock (_gate)
            {
                if (_disposed || Volatile.Read(ref _users) != 0) FatalInvariant("Destroy active recursive lock");
                _disposed = true;
            }
        }
    }
    ...
    {
        try { value->Handle = (ulong)FromContext(context).AddResource(new PlatformLock()); }
        catch (OutOfMemoryException) { FatalInvariant("Lock initialization allocation failure"); }
    }
    ...
    private static void PlatformLockDelete(void* context, CXPLAT_LOCK* value) { FromContext(context).ReleaseResource<PlatformLock>((void*)value->Handle); value->Handle = 0; }

    private sealed class PlatformRwLock : IDisposable
    {
        private readonly ReaderWriterLockSlim _value = new(LockRecursionPolicy.SupportsRecursion);
        internal void Enter(bool write)
        {
            try { if (write) _value.EnterWriteLock(); else _value.EnterReadLock(); }
            catch (Exception exception) { FatalInvariant("Invalid RW lock acquire: " + exception); }
        }
        internal void Exit(bool write)
        {
            try { if (write) _value.ExitWriteLock(); else _value.ExitReadLock(); }
            catch (Exception exception) { FatalInvariant("Invalid RW lock release: " + exception); }
        }
        public void Dispose()
        {
            try { _value.Dispose(); }
            catch (Exception exception) { FatalInvariant("Invalid RW lock cleanup: " + exception); }
        }
    }
    private static void PlatformRwInitialize(void* context, CXPLAT_RW_LOCK* value)
    {
        try { value->Handle = (ulong)FromContext(context).AddResource(new PlatformRwLock()); }
        catch (OutOfMemoryException) { FatalInvariant("RW lock initialization allocation failure"); }
    }
    ...
    private static void PlatformRwDelete(void* context, CXPLAT_RW_LOCK* value) { FromContext(context).ReleaseResource<PlatformRwLock>((void*)value->Handle); value->Handle = 0; }
    ...
    private sealed class PlatformEvent(bool manual, bool signaled) : IDisposable
    {
        ...
        internal void Set(bool value) { lock (_gate) { Check(); _signaled = value; if (value) Monitor.PulseAll(_gate); } }
        private void Check() { if (_disposed) FatalInvariant("Operation on disposed event"); }
        internal bool Wait(uint timeout)
        {
            ...
            lock (_gate)
            {
                ...
                try
                {
                    ...
                    if (!manual) _signaled = false;
                    return true;
                }
                finally { _waiters--; }
            }
        }
        public void Dispose() { lock (_gate) { if (_disposed || _waiters != 0) FatalInvariant("Destroy event with active waiters"); _disposed = true; } }
    }
    ...
    {
        try { value->Handle = (ulong)FromContext(context).AddResource(new PlatformEvent(manual != 0, initial != 0)); }
        catch (OutOfMemoryException) { FatalInvariant("Event initialization allocation failure"); }
    }
    ...
    private static void PlatformEventDelete(void* context, CXPLAT_EVENT* value) { FromContext(context).ReleaseResource<PlatformEvent>((void*)value->Handle); value->Handle = 0; }
    ...
    private sealed class PlatformThread : IDisposable
    {
        ...
        internal PlatformThread(CXPLAT_THREAD_CONFIG config)
        {
            ...
            Thread = new Thread(() =>
            {
                try { callback((void*)context); }
                catch (Exception exception) { FatalInvariant("Exception escaped translated worker: " + exception); }
            }) { IsBackground = true, Name = config.Name == null ? "msquic" : PlatformText(config.Name) };
        }
        internal void Join()
        {
            if (Thread == global::System.Threading.Thread.CurrentThread || !Started) FatalInvariant("Invalid worker self-join or unstarted thread");
            Thread.Join();
            Joined = true;
        }
        public void Dispose() { if (Started && (!Joined || Thread.IsAlive)) FatalInvariant("Delete unjoined worker thread"); }
    }
    private static uint PlatformThreadCreate(void* context, CXPLAT_THREAD_CONFIG* config, ulong* output)
    {
        if (output == null || config == null || config->Callback == null) return Status.InvalidParameter;
        *output = 0;
        if ((config->Flags & ~1) != 0) return Status.NotSupported; // Ideal processor is a hint, not affinity.
        var host = FromContext(context);
        PlatformThread? thread = null;
        void* token = null;
        try
        {
            thread = new PlatformThread(*config);
        ...
        {
            *output = 0;
            if (token != null) host.ReleaseResource<PlatformThread>(token);
    ...
    private static void PlatformThreadDelete(void* context, ulong* token) { FromContext(context).ReleaseResource<PlatformThread>((void*)*token); *token = 0; }
}
