...
public sealed unsafe partial class MsQuicHost
{
    ...
    public long DatagramSendErrors => Interlocked.Read(ref datagramSendErrors);
    public long DatagramReceiveErrors => Interlocked.Read(ref datagramReceiveErrors);
    public int OutstandingDatagramReceives => datagramReceives.Count;
    ...
    internal static void DatagramAddress(IPEndPoint endpoint, QUIC_ADDR* address, bool mapped = false)
    {
        *address = default;
        ...
        if (!mapped && ip.IsIPv4MappedToIPv6) ip = ip.MapToIPv4();
    ...
    { try { var ep = DatagramEndpoint(input); DatagramAddress(ep, output); } catch (Exception e) { FatalInvariant(e.ToString()); } }
    private static void DatapathToMapped(void* context, QUIC_ADDR* input, QUIC_ADDR* output)
    { try { var ep = DatagramEndpoint(input); DatagramAddress(ep, output, true); } catch (Exception e) { FatalInvariant(e.ToString()); } }

    private static uint DatapathInitialize(void* context, uint clientLength, CXPLAT_UDP_DATAPATH_CALLBACKS* udp,
        CXPLAT_TCP_DATAPATH_CALLBACKS* tcp, CXPLAT_WORKER_POOL* workers, CXPLAT_DATAPATH_INIT_CONFIG* config, CXPLAT_DATAPATH** output)
    {
        if (output == null) return Status.InvalidParameter;
        *output = null;
        if (udp == null || udp->Receive == null || udp->Unreachable == null || workers == null) return Status.InvalidParameter;
        if (tcp != null || (config != null && (config->EnableDscpOnRecv != 0 || config->XdpMapConfigCount != 0))) return Status.NotSupported;
        DatagramPath? path = null;
        try
        {
            var host = FromContext(context);
            if (clientLength > int.MaxValue - 4096) return Status.OutOfMemory;
            path = new DatagramPath(host, *udp, workers, checked((int)clientLength));
            *output = (CXPLAT_DATAPATH*)host.AddResource(path);
            return Status.Success;
        }
        catch (Exception error) { path?.Dispose(); return DatagramStatus(error); }
    }
    private static void DatapathUninitialize(void* context, CXPLAT_DATAPATH* path)
    { try { FromContext(context).ReleaseResource<DatagramPath>(path); } catch (Exception e) { FatalInvariant(e.ToString()); } }
    ...
    { if (output == null) return Status.InvalidParameter; *output = null; return Status.NotSupported; }
    private static void DatapathRssFree(void* c, CXPLAT_RSS_CONFIG* value)
    { if (value != null) FatalInvariant("RSS configuration was not allocated by this host."); }
    private static void DatapathPolling(void* c, CXPLAT_DATAPATH* p, uint timeout)
    { if (timeout != 0) FatalInvariant("Busy-polling is outside the BCL datapath profile."); }
    ...
    private static uint DatapathResolveAddress(void* c, CXPLAT_DATAPATH* p, byte* name, QUIC_ADDR* output)
    {
        ...
        catch (Exception e) { return DatagramStatus(e); }
    }
    private static uint DatapathLocalForRemote(void* c, QUIC_ADDR* remote, QUIC_ADDR* local)
    {
        if (remote == null || local == null) return Status.InvalidParameter;
        try
        {
    ...
    {
        if (output == null || count == null) return Status.InvalidParameter;
        *output = null; *count = 0;
        try
        {
            ...
            foreach (var adapter in NetworkInterface.GetAllNetworkInterfaces())
                foreach (var unicast in adapter.GetIPProperties().UnicastAddresses)
                {
                    ...
                    if (ip.AddressFamily is not (AddressFamily.InterNetwork or AddressFamily.InterNetworkV6)) continue;
                    CXPLAT_ADAPTER_ADDRESS item = default;
                    DatagramAddress(new IPEndPoint(ip, 0), &item.Address);
                    item.InterfaceIndex = checked((uint)(ip.AddressFamily == AddressFamily.InterNetwork
                        ? adapter.GetIPProperties().GetIPv4Properties().Index : adapter.GetIPProperties().GetIPv6Properties().Index));
            ...
            var memory = (CXPLAT_ADAPTER_ADDRESS*)FromContext(c).AllocatePlatformMemory(checked((ulong)list.Count * (ulong)sizeof(CXPLAT_ADAPTER_ADDRESS)), unchecked((uint)MsQuic.QUIC_POOL_DATAPATH_ADDRESSES));
            if (memory == null && list.Count != 0) return Status.OutOfMemory;
            for (int i = 0; i < list.Count; i++) memory[i] = list[i];
            *output = memory; *count = checked((uint)list.Count); return Status.Success;
        }
        catch (Exception e) { return DatagramStatus(e); }
    }
    ...
    {
        if (output == null || config == null) return Status.InvalidParameter;
        *output = null;
        ...
        if ((config->Flags & ~supported) != 0 || config->CibirIdLength != 0) return Status.NotSupported;
        ...
        {
            if (socket != null && socket.Token != null) FromContext(context).ReleaseResource<DatagramSocket>(socket.Token);
            else socket?.Dispose();
            return DatagramStatus(e);
        }
    }
    private static void DatapathSocketDelete(void* c, CXPLAT_SOCKET* p)
    { try { var h = FromContext(c); h.Resource<DatagramSocket>(p).Close(true); } catch (Exception e) { FatalInvariant(e.ToString()); } }
    ...
    {
        if (route == null) return Status.InvalidParameter;
        if (route->DatapathType == 2 || route->UseQTIP != 0) return Status.NotSupported;
        try
        {
    ...
    { if (route == null || route->DatapathType == 2 || route->UseQTIP != 0) FatalInvariant("Raw route completion is unsupported."); route->State = CXPLAT_ROUTE_STATE.RouteResolved; }
    private static void DatapathUpdateRoute(void* c, CXPLAT_ROUTE* target, CXPLAT_ROUTE* source)
    { if (target == null || source == null || source->DatapathType == 2 || source->UseQTIP != 0) FatalInvariant("Raw route update is unsupported."); *target = *source; }
