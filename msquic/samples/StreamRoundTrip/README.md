...
```bash
dotnet build msquic/samples/StreamRoundTrip/StreamRoundTrip.csproj -c Release
```
...
```bash
dotnet msquic/samples/StreamRoundTrip/bin/Release/net10.0/PublicQuicSample.dll certificates /tmp/quic-sample-certificates
```
...
```bash
dotnet msquic/samples/StreamRoundTrip/bin/Release/net10.0/PublicQuicSample.dll server /tmp/quic-sample-certificates/server.pem /tmp/quic-sample-certificates/server-key.pem 127.0.0.1 45678 /tmp/quic-sample-ready
```

Run the client in another terminal:

```bash
dotnet msquic/samples/StreamRoundTrip/bin/Release/net10.0/PublicQuicSample.dll client /tmp/quic-sample-certificates/root.pem localhost 127.0.0.1 45678
```
...
```bash
dotnet publish msquic/samples/StreamRoundTrip/StreamRoundTrip.csproj -c Release -r linux-x64 --self-contained true -p:PublishAot=true -o msquic/artifacts/public-sample-aot
```
