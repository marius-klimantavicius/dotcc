# System.Net.Quic messages in one app

Run from the repository root with .NET 10 and the existing generated MsQuic and
PicoTLS sources:

```sh
dotnet run --project msquic/samples/SystemNetQuicRoundTrip -c Release
```
...
`Program.cs` contains the listener, client, and message exchange. The
`ManagedQuic` reference alias selects our translated `System.Net.Quic` API.
