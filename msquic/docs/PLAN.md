...
unqualified, including one unmatched harness-deadline outcome; this is not an
unconditional P7/P9 pass. See [qualification](qualification.md).
...
| --- | --- |
| P0 | Passed: immutable source integrity verified; native 44-unit syntax control passes. Fresh native peer validation passes 8 certificate/cipher/IP cases and 2 authentication negatives. The refreshed pinned independent aioquic cross-connect passes both roles in 16 positive and 4 negative cases. The validated 47-unit source/host closure is pinned in `config/product-closure.json` with exact source, compiler, generated and evidence hashes. |
| P1 | Passed: [UDP feasibility](datapath-feasibility.md) passes 14 cases in JIT and AOT; [raw picotls QUIC TLS](tls-feasibility.md) passes 11 cases in each raw/optimized × JIT/AOT combination. Public upstream ABI matches 29 records in JIT/AOT. The managed 99-slot host contract, TLS and complete core ABI match all 60 native records under JIT/AOT. Feasibility and ABI subgates pass. |
| P2 | Passed: [compiler implementation](compiler-implementation.md) records validated preprocessing, declarations, anonymous members, alignment storage, high-bit conversions, and GNU intrinsics. The complete staged core emits, links and passes JIT/AOT ABI checks. Full unit 2146 and executed functional 447 pass (1005 optional oracle skips); Raw and optimized managed libraries and whole-assembly-rooted JIT/NativeAOT consumers pass. Runtime host services remain separate gates. Generic promoted-member value accessors are committed as `c786b9c`; the new compiler and complete raw/optimized closure pass refreshed 60 host/core and 29 public native ABI records plus whole-assembly JIT/AOT consumers. The prior checkpoint remains preserved. |
| P3 | Passed in raw/optimized × JIT/NativeAOT: actual workers, native status mapping and all 11 allocation rollback positions. The complete transport and owning API matrices also verify global rundown and owner drain. See `artifacts/platform-host/results.json` and [worker lifetime](worker-lifetime.md). |
| P4 | Passed in all four forms: packet crypto has 162 checks per form plus native controls; the actual CxPlatTls adapter has 20 cases per form, including imported-key resumption and credential paths. Fresh translated picotls regression, complete transport and facade certificate-policy tests also pass. See `artifacts/packet-crypto/results.json`, `artifacts/tls-adapter/results.json` and [TLS adapter contract](tls-adapter-contract.md). |
| P5 | Final frozen closure UDP service refresh passed raw/optimized × JIT/NativeAOT, including the corrected non-segmented size-hint contract, actual worker completions, addresses and buffer/socket lifetimes. See [datapath host](datapath-host.md) and `artifacts/datapath-host/results.json`. The typed host table injection controls now pass all 40 cases across four forms and both IP families. Actual-core keepalive/idle qualification passes 48 controls across both ciphers, IP families and disabled/client/server keepalive profiles (`keepalive_qualified=true` in its separate receipt). |
| P6 | Fresh raw/optimized × JIT/NativeAOT basic transport passes all 80 pairs: 16 translated-to-translated and 64 native interoperability cases across both ciphers, certificate types and IP families, with exact bidirectional 65,537-byte payloads, FIN, authenticated profile and clean resource drain. Binary/source hashes are recorded in `artifacts/managed-peer/results.json`. Authentication negatives and broader stream/lifetime qualification remain in the dependent API/independent-peer gates. The test-only shared-binding CID profile is included in refreshed peer binaries; ordinary cases explicitly verify default binding. |
| P7 | Passing selected gates: 75 native-matched decoder cases in each of four forms; 64 live malformed-input/amplification profiles; 40 injected host failures; 48 actual keepalive/idle controls; 480 positive fault-recovery cases; 88 independent interoperability cases; and 152 CID profiles. [Recovery evidence](recovery-campaign.md) also completes 192 fixed stronger-check observations plus 48 native positive controls, producing 80 comparisons. Dual-live rebinding retains native 19 successes / 61 failures and managed 13 / 67; these frequencies are not claimed equivalent. Decreasing-ceiling delivery fails all 32 observations: native 16 idle closures, managed 15 idle closures and one explicit harness FIN deadline in optimized NativeAOT / IPv4 / AES-256. That terminal kind is unmatched. The stronger checks remain unqualified; `complete_evidence_validated=true` does not promote `observation_validated=false` or `strict_passed=false`. This is not a full upstream-suite or unconditional P7 pass. |
| P8 | Passed: the coherent `artifacts/managed-api-all/results.json` covers all 17 modes in four forms (68 executions), including stream/receive ownership, cancellation, registration shutdown, callbacks, settings, versions, packet features, certificate policy, resumption and stateless secret binding/rotation. Actual runtime revision queries match the pinned source. A test-only reset-arrival assumption was repaired without weakening exact transport-status, counter or owner-drain assertions; the original failed run remains archived. The separate 32-case public consumer matrix passes JIT and trimmed NativeAOT with ordinary project references, authentication negatives, resumption and clean shutdown. [API documentation](managed-api-design.md) and [coverage](api-coverage.md) describe the selected surface and exclusions. |
| P9 | Evidence collection completed: final SQLite/compiler regression and normal optimized SQLite restoration, fresh picotls regression, regenerated 47-unit closure, actual source metadata, service gates, 80 transport pairs, 68 API executions and 32 separate-consumer cases pass. The dependency audit has zero violations or missing prerequisites. Fresh dependent 88-case independent, 152-case CID and 64-profile endpoint campaigns pass after the test-only FIN/cleanup barriers. All 480 positive recovery cases pass. The full 20-pair benchmark includes shutdown and owner-disposal durations; [performance investigation](performance.md) reports measured implementation tradeoffs and a preserved 30-second trace without unsupported CPU attribution. [Qualification](qualification.md) preserves the completed but unmatched stronger-recovery observations; they prevent an unconditional P7/P9 qualification claim. No required matrix remains unexecuted within this recorded Linux x64 campaign. |
