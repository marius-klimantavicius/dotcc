...
| P8 | Owning API builds cleanly. Optimized JIT controls pass runtime/configuration validation, IPv4/IPv6 stream transport and lifetime cases (partial receives, canceled waits, close races), and authenticated ticket resumption with asynchronous policy approval/rejection. Packet/key-update/Retry/reset controls and a separate public consumer are authored but unexecuted. Full raw/optimized × JIT/NativeAOT and exact pinned revision metadata remain required. See `artifacts/managed-api/results.json`, `artifacts/managed-api-transport/results.json`, and `artifacts/managed-api-resumption/results.json`. |
| P9 | Final clean regeneration, complete regression/audit/performance campaign and fresh SQLite JIT/AOT correctness validation remain pending. Final stage defines must set `VER_GIT_HASH` from the pinned source commit, then refresh the 47-object closure and every dependent receipt: current upstream build metadata reports `Unknown`, so exact source-revision API qualification is still open. |
