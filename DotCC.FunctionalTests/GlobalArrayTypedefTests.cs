...
public sealed partial class ManagedLibraryTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void Thread_local_array_typedef_keeps_distinct_pinned_storage_through_gc(bool objectLink)
    {
        var fixture = FixtureRunner.Discover().Single(row => row.name == "tls-array-typedef");
                ...
                public static unsafe class Consumer
                {
                    private static void Worker(int index, System.Threading.Barrier barrier, nint[] addresses, int[] errors)
                    {
                        ...
                        for (int pass = 0; pass < 3; ++pass)
                        {
                            ...
                            if (DotCcLib.get_tls() != pointer || pointer[0] != value || pointer[1] != value + 1)
                                errors[index] = 4;
                        }
                    ...
                    public static int Run()
                    {
                        ...
                        System.GC.Collect(2, System.GCCollectionMode.Forced, true, true);
                        return DotCcLib.get_tls() == main && main[0] == 3 && main[1] == 4 ? 42 : -5;
                    }
                }
                """, references);
