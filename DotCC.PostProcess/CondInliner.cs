...
public static class CondInliner
{
    public static RewriteResult Rewrite(CSharpCompilation compilation, CancellationToken cancellationToken = default)
    {
        ...
        var helper = compilation.GetTypeByMetadataName("Cond");
        if (helper is null) return new(compilation, 0, 0, []);
        var diagnostics = new List<string>();
        // Removing a call must not remove a type initializer or field effects.
        ...
        foreach (var tree in compilation.SyntaxTrees)
        {
            ...
            var visitor = new Rewriter(compilation.GetSemanticModel(tree), helper, pureContainer, diagnostics, cancellationToken);
    ...
    private sealed class Rewriter(SemanticModel model, INamedTypeSymbol helper, bool pureContainer,
        List<string> diagnostics, CancellationToken token) : CSharpSyntaxRewriter
    {
        ...
        private static ExpressionSyntax? Body(IMethodSymbol method)
        {
            ...
            {
                MethodDeclarationSyntax m => m.ExpressionBody?.Expression ?? (m.Body?.Statements is [ReturnStatementSyntax r] ? r.Expression : null),
                ConversionOperatorDeclarationSyntax c => c.ExpressionBody?.Expression ?? (c.Body?.Statements is [ReturnStatementSyntax r] ? r.Expression : null),
                _ => null
            };
        }
        ...
        private bool CheckNormalizer(IMethodSymbol normalizer, ITypeSymbol type)
        {
            ...
            if (Operation(body) is not IObjectCreationOperation { Arguments.Length: 1, Constructor: { } constructor } creation
                ...
                { Initializer: null, Body.Statements: [ExpressionStatementSyntax { Expression: AssignmentExpressionSyntax assignment }] }
