        ...
        CSharpCompilation optimized, string outputDirectory)
    {
        ...
        string WriteVariant(string directory, string projectName, SyntaxTree[] trees)
        {
            ...
            var semantics = new XElement("PropertyGroup",
                ...
                Property("NoWarn", string.Join(";", options.SpecificDiagnosticOptions.Where(pair => pair.Value == ReportDiagnostic.Suppress).Select(pair => pair.Key))),
                Property("WarningsAsErrors", string.Join(";", options.SpecificDiagnosticOptions.Where(pair => pair.Value == ReportDiagnostic.Error).Select(pair => pair.Key))),
                Property("WarningsNotAsErrors", string.Join(";", options.SpecificDiagnosticOptions.Where(pair => pair.Value == ReportDiagnostic.Warn).Select(pair => pair.Key))),
