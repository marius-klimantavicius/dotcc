...
/* dotcc's <stdarg.h> — C99 7.15. Variadic functions.
   ...
   trailing `params ReadOnlySpan<VaArg>` parameter; C# converts each variadic actual to a
   ...
   the emitter:
     ...
     va_start(ap, last);         -> ap = new VaList(_va);   (the params span)
...
/* va_list — a cursor over a variadic function's borrowed argument pack. VaList is a C#
   ref struct reachable through `using static Libc;`; it's pre-registered as a
