...
internal sealed partial class CSharpBackend
{
    private void AddMacroConstants(IrBuilder unit, IDictionary<string, string> declarations)
    {
        foreach (var group in unit.MacroConstants.GroupBy(m => m.Name).OrderBy(g => g.Key, StringComparer.Ordinal))
        {
            ...
            var fields = group.Select(m =>
            {
                var field = MacroField(name, m.Value);
                if (m.Selected && field == null)
                    throw new CompileException($"cannot export selected macro '{m.Name}': value cannot be represented as a C# field");
                return field;
    ...
    private string? MacroField(string name, CExpr? value)
    {
        ...
        if (value.Type.Unqualified is CType.Prim { Name: "_Bool" })
            return $"    public static readonly {Cs(value.Type)} {name} = ({Cs(value.Type)})({Expr(value)});\n";
        if (value.Type.Unqualified is CType.Pointer or CType.Func)
            return $"    public static readonly unsafe {Cs(value.Type)} {name} = unchecked({Expr(value)});\n";
        if (value.Type.Unqualified is CType.Enum)
            return $"    public const {Cs(value.Type)} {name} = unchecked({Expr(value)});\n";
